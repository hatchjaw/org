...
standard for the transmission of multichannel digital
audio\nbsp[cite:@bakkerIntroduction2014]. Audio over ethernet 
...
(STMicroelectronics[fn:8]) development boards.

Additionally, many of these platforms are supported by the Faust
programming language, meaning that DSP code can be written once and
compiled to run on a variety of embedded platforms
[cite:@michonEmbedded2020].
...
Faust, and /even possessing a SoC with PTP support/, but having no
Ethernet PHY,[fn:2] and thus no way to connect it to a network. There
is a sense in which, short of designing and manufacturing, at scale, a
...
of audio devices was subject to testing. The test procedure is given
...
\qty{1}{\micro\s}, corresponding to a propagation discrepancy of less than
half a millimetre; we have improved from a scale comparable with the
height of an adult human --- sufficient to obliterate the integrity of
any attempt at sound field synthesis --- to an order of magnitude
below that of the diameter of the eardrum.
