...
however, to paralellise sound field synthesis algorithms and
...
embedded hardware platforms to have emerged over the past decade or
so, and open-source software implementing the network Precision Time
Protocol, we show that low-cost, networked audio devices can be
synchronised to a degree sufficient to support time-sensitive audio
applications.
#+end_abstract
...
wishes for numerous participants to share in a participatory auditory
experience, unencumbered by headsets, an immersive acoustic
environment is desirable, and yet may lie out of reach, for practial
...
loudspeakers. To minimise spatial
aliasing\nbsp[cite:@winterGeometric2018], WFS demands a dense array of
secondary point sources; to the same end, ambisonics requires a number
of speakers sufficient to reproduce spherical harmonics of higher
...
DSP\nbsp[cite:@noisternigDispositif2012;@baalmanRenewed2007];[fn:9] in either
case, conservative cost estimates on the order of hundreds of euros
per channel may be reached\nbsp[cite:@rushtonNetworked2024].
...
The rise of embedded computing, devices of the /internet of things/
variety, and initiatives in the DIY and maker communities, have given
rise to alternative approaches. With frugality in mind, a modular,
FPGA-based WFS system has been demonstrated, programmed in Faust and
using high-quality, low-cost audio codecs\nbsp[cite:@michonMaking2023]; a
fully-costed DIY ambisonics dome, with 3D-printed speaker enclosures,
is another recent contribution\nbsp[cite:@mitterhuberOttosonics2022].
Both WFS and ambisonics have been attempted in distributed form, on
GPUs\nbsp[cite:@bellochPerformance2021] and
MCUs\nbsp[cite:@rushtonMicrocontrollerbased2023], with ambisonics further
supported by the use of an MCU supporting the audio video bridging
(AVB) protocol suite for audio processor
synchronisation\nbsp[cite:@devonportDistribution2019].

AoE democratised multichannel audio... ironic then that Dante and L-Acoustics...
...
Ethernet has, for the better part of the past twenty-five years, been
the de facto standard for the transmission of multichannel digital
audio\nbsp[cite:@bakkerIntroduction2014]. Audio over ethernet largely
replaced prior analogue point-to-point systems
[cite:@cochardEthernet2024]

** Precision Time Protocol

IEEE 1588, Precision Time Protocol (PTP), is an ethernet protocol for
precise clock synchronisation between networked devices... Though
often associated with more expensive networking equipment, support for
PTP is offered by a number of low-cost MCUs and SoCs, including the Broadcom
...
The past decade or so has seen the emergence of a raft of low-cost,
embedded hardware platforms, either providing audio support, or having
audio as a key part of their /raison d'être/. Amongst these platforms
are a selection based on Embedded Linux Systems (ELS) such as
/Bela/\nbsp[cite:@mcphersonBela2017] and /Elk/\nbsp[cite:@turchetElk2021],
companion boards for the BeagleBone Black[fn:4] and Raspeberry
Pi[fn:5] respectively. These examples profit from the power of the
underlying ELS, with a system-on-chip (SoC) providing a multicore CPU
and peripherals such as memory and connectivity via, e.g., USB and
ethernet. A separate subset based on microcontroller (MCU)
systems,[fn:6] prominent amongst these being the
...
The particular requirements of the distributed spatial audio case,
however, mean that not all such platforms, details of a selection of
which are gathered in cref:tab:platforms, are suitable to the
task. One may find, for example, that a sub-€30 development board such
as the /Daisy Seed/, well-known to the DIY/maker audio community,
...
and is memory rich, but clock dividers...
And the switch...
  There is a sense in which,
short of designing and manufacturing, at scale, a printed circuit
board of one's own, the stars must align.
...
\qty{1}{\ns\per\s} at a nominal \qty{48}{\kHz} corresponds to about one
twenty-one-thousandth of a cycle of the audio clock. 

Being descended from the same internal reference clock --- a
\qty{24}{\MHz} crystal oscillator --- we can be sure that conclusions
drawn about the notion of time at the level of the PTP timer are also
applicable to the audio subsystem.
...
To evaluate the proposed audio clock conditioning algorithm, a network
of devices was subject to testing. The test procedure is given
in the section to follow; this is followed in turn by a discussion of
the results yielded.
...
MikroTik device can function effectively in such a configuration. We
lie at the mercy of the market in this regard; an switch running on
open-source software, and perhaps an FGPA, represents a long-term
objective.
...
impulse resposes, and sufficiently performant access to that memory
...
[fn:9] For the /logical conclusion/ to this technological story,
consult HOLOPLOT's behemoth installation at /The Sphere/, Las Vegas:
