...
conncted modules, which perform their part of a distributed spatial
audio algorithm.
...
#+end_src
#+name: fig:synchro
#+caption: Synchronisation measurements for a group of four Teensy modules reproducing a pulse-per-second audio signal.
...
cooridinates of the virtual sound sources to produce their part of the
...
at all times (see cref:fig:synchro).
...
and familiar digitial audio workstatiton tools make it approachable to
