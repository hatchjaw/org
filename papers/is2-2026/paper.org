...
Code for the Ananas server can be found at
\url{https://github.com/hatchjaw/ananas-server}; code for the Ananas
client and time authority can be found at
\url{https://github.com/hatchjaw/ananas-client}.
...
#+begin_src julia :results file graphics :file histo.png :output-dir ./images/gen :exports results
using CSV, DataFrames, Plots, LaTeXStrings, Statistics
...
histogram(Matrix(data)[:, 1], bins=30,
          fontfamily="Times-Roman",
          xlabel="Offset (ns)",
          ylabel="Frequency",
          label="192.168.198.18")

savefig("./images/gen/histo.png")
#+end_src
#+name: fig:histo
#+caption: Histogram of synchronisation measurements for one of the
#+caption: clients in cref:fig:synchro.
#+attr_org: :width 300
#+RESULTS:
[[file:./images/gen/histo.png]]
...
computer, runs the Reaper[fn:4] digital audio workstation with a CLAP
format WFS control plugin assigned to a 16-channel track, and sends
monophonic audio streams to a UDP multicast group managed by a single
switch, which forwards the data on to the hardware modules. Eight
modules are used in this setup for a total of sixteen output
channels. The server sending packets containing sixteen frames of
audio data, it is possible to transmit up to 44 channels, each
representing either a WFS virtual source or an ACN-format channel; up
to fifth order HOA is supported.

Thanks to the media clock recovery algorithm described in
cref:sec:media-clock, modules remain synchronised to within around
\qty{200}{\nano\second} at all times. Cref:fig:synchro shows an hour's
worth of synchronicity measurements for a group of four hardware
modules. The modules were connected to a Digilent Analog Discovery 2
logic analyser and their I^{2}S output measured in Digilent Waveforms
software. The server delivered a pulse-per-second audio signal and
offsets were measured at the rising edge of the pulse. The mean
asynchronicity of \sim\qty{80}{\nano\second} falls well within the AES
guideline for time-sensitive applications of 5% of the reference frame
(\sim\qty{1}{\micro\second} at \qty{48}{\kilo\hertz}) and corresponds with a
propagation discrepancy in air of roughly \qty{27.4}{\micro\meter}. A
histogram of synchronisation data for client =192.168.198.18= is
presented in cref:fig:histo.
...
available on each hardware module via time-division multiplexing,
