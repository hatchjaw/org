...
\begin{bytefield}[bitwidth=.55cm, bitheight=.55cm]{16}
      ...
      \bitbox[lr]{4}{\scriptsize{Presentation offset frames}} & \bitbox[lr]{8}{Presentation offset ns} & \bitbox[lr]{4}{\scriptsize{Audio-PTP offset ns}} \\
      \bitbox[lrtb]{1}{\tiny{Buffer fill \%}} & \bitbox[lrtb]{1}{\tiny{PTP lock}} & \bitbox[ltb]{14}{Secondary source co-ordinates} \\
      \bitbox[rb]{2}{}
    \end{bytefield}
\end{figure}
...
on the server machine and PTP time for the switch and other connected
modules. The server sends audio and control data to the switch, which
forwards this data on to the conncted modules, which perform their
part of a distributed spatial audio algorithm.
