...
This =org= file describes my presentation for the /doctorants/ day in
May 2026.
...
Then navigate to [[http://localhost:8000/doctorants-2026/]].
...
* Figures                                                          :noexport:
:PROPERTIES:
:header-args: :tangle figs.sty :results silent :noeval
:END:
...
WellenFeld H 104, TU Berlin, 2700 loudspeakers
...
#+ATTR_REVEAL: :frag (appear)
- Spatial audio has an accessibility problem.
- Installation represents a significant investment in money, time and
  expertise.
...
#+ATTR_REVEAL: :frag (appear)
- One Teensy acts both as a Precision Time Protocol (PTP) time
  authority /and/ as a USB audio device.
...
#+header: :headers '("\\usepackage{figs}")
