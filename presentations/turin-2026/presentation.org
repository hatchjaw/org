...
\tikzset{
  ...
  moduleN/.pic = {
    ...
    \draw [bformat] (-.25, .6) -- ([xshift=-.25cm]-body.north);
    \draw [ctrl] (.25, .6) -- ([xshift=.25cm]-body.north);
  },
  moduleS/.pic = {
    \node [module] (-body) {dHOA};
    \draw [ptp] (0, -.6) -- (-body.south);
    \draw [bformat] (-.25, -.6) -- ([xshift=-.25cm]-body.south);
  ...
  hoa/.pic={
    ...
    \begin{scope}[yshift=5.5cm]
      ...
      % Coordinates
      \draw [ctrl] (1, -1.5) -- ([xshift=1cm] switch.south);
      \draw [-{Triangle[open]}, ctrl] ([xshift=1cm] switch.north) -- (1, 1.5);
    \end{scope}
    \begin{scope}
      % DAW
      \draw [thick] (-4, -4) rectangle (4, 4);
