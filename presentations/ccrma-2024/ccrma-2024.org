...
#+REVEAL_EXTRA_SCRIPTS: ("dist/ccrma.umd.js")
...
#+begin_src json :tangle package.json
{
  ...
  "dependencies": {
    "@grame/faust-web-component": "github:hatchjaw/faust-web-component#a85d131231ee95b0b94373433a71b85cf5c47743"
  }
}
#+end_src

The above was actually created by running ~npm create vite@latest~,
then:
...
export default defineConfig({
    build: {
        ...
        lib: {
            ...
            name: "ccrma",
            formats: ['umd'],
            fileName: format => `ccrma.${format}.js`,
        },
        sourcemap: true,
    }
});
...
Wait till Reveal.js is ready before loading ~faust-editor~; other wise
Split's parent element may not yet have a size, and the tab buttons
will hang over the edge.

#+begin_src js
import FaustEditor from '@grame/faust-web-component/src/faust-editor'
...
Exporting to reveal.js =C-c C-e v v= triggers ~npm run build~ (see
[[Emacs setup]])
...
Then navigate to [[localhost:8000/ccrma-2024/]]. This offers nice things
like livereloading.
...
# (interactive)
# (message "building js...")
# (shell-command-to-string "npm install && npm run build")))
# End:
