...
But not in code blocks, and don't use ligatures.
...
#+ATTR_REVEAL: :frag t
The nanoseconds-per-second adjustment, \(a_{n}\), is found as
...
- Solution: take a reference nanoseconds figure on audio system
  startup, \(t_{S}\), and compare it with the nanosecond part of PTP
  time, \(t_{\text{PTP}}\), once per second (only works if the buffer
  size is an integer divisor of the sampling rate)
  
#+ATTR_REVEAL: :frag t
\begin{equation}
\Delta = t_{\text{PTP}} - t_{S}
\end{equation}
...
#+ATTR_REVEAL: :frag t
\begin{equation}
\hat{f}_{s,n} = f_{s}(1 + a_{n} + K_{P_{\Delta}}\Delta_{n} + K_{I_{\Delta}}\sum_{t=0}^{n}\Delta)
\end{equation}
