...
* Figures                                                          :noexport:
:PROPERTIES:
:header-args: :results silent :noeval
:END:
...
** Teensy
:PROPERTIES:
:header-args: :tangle teensyfigs.sty :results silent :noeval
:END:


For some reason I need to scale the font aggressively to get nice
output. I'm probably using =imagemagic= wrong.
...
Draw a Teensy with a thought bubble, and an optional magnifying glass.
...
\tikzset{
  pics/teensythinks/.style={
    code={
      ...
      \node [] at (0,0) {
        \includegraphics[width=40cm]{./images/t41_orig}
      };
      \draw [line width=5mm, fill=white] (3,10) ellipse (2cm and 1cm);
      \draw [line width=5mm, fill=white] (0,13.5) ellipse (3.5cm and 1.75cm);
      \node [cloud, cloud ignores aspect, cloud puffs=9, cloud puff arc=96, draw, line width=5mm, fill=white, align=center] () at (-9, 23) {\nomFS\ kHz!};

      \pgfmathparse{\actFS > 0}
      \ifnum\pgfmathresult=1
        \node [magnifying glass, line width=5mm, draw, fill=white, fill opacity=0.75, text opacity=1, align=center] () at (6.5,-6.5) {\actFS\\[.75ex]kHz};
      \fi
    }
  },
  teensythinks/.cd,
...
** WFS
:PROPERTIES:
:header-args: :tangle wfsfigs.sty :results silent :noeval
:END:

No font scaling required here.

#+begin_src latex
\ProvidesPackage{wfsfigs}
\usepackage{tikz}
\usepackage{fontspec}
\setmainfont[
  Mapping={tex-text}, 
...
#+header: :headers '("\\usepackage{wfsfigs}")
...
#+header: :imagemagick t :iminoptions -density 100 :imoutoptions -geometry 400 -flatten :fit t
...
#+header: :imagemagick t :iminoptions -density 100 :imoutoptions -geometry 1000 -flatten :fit t
#+header: :headers '("\\usepackage{teensyfigs}")
#+begin_src latex :eval no-export
\begin{tikzpicture}
  \pic at (0, 0) {teensythinks={nominalfs=48}};
  \pic at (50, 0) {teensythinks={nominalfs=48}};
  \pic at (100, 0) {teensythinks={nominalfs=48}};
\end{tikzpicture}
#+end_src
...
\begin{tikzpicture}
  ...
  \pic at (50, 0) {teensythinks={nominalfs=48, actualfs=48.001}};
  \pic at (100, 0) {teensythinks={nominalfs=48, actualfs=47.999}};
\end{tikzpicture}
#+end_src
