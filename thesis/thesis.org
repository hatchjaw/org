...
#+begin_src latex
\usepackage[noabbrev,capitalise]{cleveref} \usepackage{hyperref}
\hypersetup{
  unicode,
  colorlinks,
  citecolor=black,
...
Guillaume, Aurélien, Mehdi, Kim, Antoine, Julien. Linda Soumari,
...
Wolska and Morgane Le Prince. Frédéric Lemouel, Léonardo Cardoso and
Jens Ahrens; Cristoff Ressi, Shaun Silverman, and Jean-Michel Friedt.
...
point-to-point installations, it was the broadcast industry that first
embraced audio over ethernet\nbsp[cite:@bakkerIntroduction2014], and
digital telephony that pioneered more general audio over internet
protocol (IP) approaches\nbsp[cite:@turlettiINRIA1994].
...
of a cycle per second; this falls far, far below the threshold for
human perception of difference in pitch. The audio engineer notes,
however, that the devices running at \(f_{s_{1}}\) and \(f_{s_{2}}\) will,
after one second, be out of sync by roughly the duration of one full
audio sample; the sum two audio signals, \(x_{1}[n]\) and \(x_{2}[n]\),
...
# org-export-filter-headline-functions: (tar/acks-as-section tar/abstract-as-section tar/intro-in-toc)
# End:
