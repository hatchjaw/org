...
#+TAGS: HARDWARE(h) INSPO(i) DIY(d) TEACHING(t) REPRES(R) REVIEW(v) GSoC(g)
...
**** READ Enhancement of cardiac and respiratory sounds for cellphone reproduction by means of digital sound processing methods :PAUC:REVIEW:
:PROPERTIES:
...
***** General notes
This is another step in the right direction, though some of the
changes made for this revision introudce new problems, which I detail
below.
...
Another thing I feel the reader is missing now is a clear sense of to
what extent the pitch of the recorded samples was altered. It would be
helpful to included a link to some before-and-after audio examples
from the open source databases, or, alternatively, in the paper, a
couple of figures containing pre- and post-processing spectrograms.

I would urge you to be a little more circumspect when discribing the
strength of your findings. Reference [25] describes a specificity of
89% for traditional auscultation; your 70-80% describes preservation
of clinical information, /not specificity/. Comparisons such as
"...similar to the values reported in the literature" (page 11) are,
as a result, potentially misleading.
...
- 2, 2, 27/28: "...coughs, allergies, and sneezes". The cited source
  is concerned with cough sensing, not sneezes, and /one/ participant
  exhibited a cough caused by an allergy --- the study does not aim to
  detect allergies per se.
...
- 3, 2, 6-9: You have "pitch can be used" twice in rapid
  succession. Also, you don't make it clear how pitch can be used as a
  timbral identifier; my guess is you're referring to the way the
  authors of reference [28] use "pitch" to describe /unpitched/ sounds
  like rhonchus, which have a low dominant frequency, but resemble
  snoring (i.e. they're inharmonic).
- 3, 2, 48-50: A frequency response may /exhibit/ attenuation, but it
  doesn't /do/ the attenuation itself. It might be better to emphasise
  something interesting alluded to in reference [31], i.e. that mobile
  phone speakers have typically been optimised for communication, not
  a flat frequency response across the audible range.
- 4, 1, 24-26: As I noted for the previous revision, pitch shifting
  ...
  "These algorithms can allow/facilitate pitch changes across a wide
  frequency range without..."
- Fig. 3: I may be wrong about what is deemed the anterior and
  posterior projections of the lungs, but the right pannel shows what
  looks like the back (posterior) of the torso and the left panel the
  front (anterior).
- 7, 2, 2-5: The target readership does not need to be told what an
  FFT is.
- Table 1, Table 2: Please indicate, in their respective captions,
  which database(s) each table refers to.
- Table 5: "Murmers represent abnormal ones" --- Not all of the
  pathologies are murmers. Why not use "Normal"/"Pathological" instead?
- 10, 1, 40: "they did hold clinical information to the ears of
  physicians" --- Is this strictly what you mean? Presumably 100% of
  the processed sounds held clinical information, but 73.3% were rated
  as maintaining the clinical information represented by their
...
comments refer to relatively minor matters.

Some of the following may come across as nitpicking on my part, but my
aim is to suggest more comfortable wording where possible.

- 3, 2, 53...: "there are sereval ways to improve these sounds to allow
  them to be played on a smartphone" \rightarrow "there are ways to optimise
  these sounds for reproduciton on a smartphone"
...
- 7, 1, 8-10: There's no need to supply mathematical notatation for
  maximum attack and release times.
...
- 7, 2, 34-35: "indepenent" \rightarrow "independently"
- Table 1: "Tracheal sound" \rightarrow "Tracheal breath sound"
- 10, 1, 40: "e.g." \rightarrow "i.e."
- 11, 1, 51-53 + 11, 2, 1-6: This sentence is very long, and difficult
  to follow. At the very least, try adding a comma after "for them"
  and "pathological lung sounds". (As stated above, you may be best
  advised to reword this sentence altogether.)
- 12, 1, 38: "...to compare, in future studies, the diagnostic..."
- References: Fine for the most part; [36] features some bonus
  quotation marks, e.g. /Shelvock"/ and /"2012"/
