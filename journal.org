...
approach. It would also be nice to decouple packet size from buffer
size. I also need to change from sample- to channel-interleaved
format. There's no reason, per-se, that packets should necessarily
carry a power-of-two number of frames; like AOO, packets should be
arbitrarily fragmentable.
