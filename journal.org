...
with changing audio device setup. Basically I don't call
~AudioTransportSource::prepareToPlay~ or ~AnanasServer::prepareToPlay~
until the expected number of frames per block is as I expect it;
otherwise I think the outgoing packet is initialised, then later
resized, and that can sometimes result in memory that's sufficiently
mangled to cause a segfault.
