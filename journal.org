...
#+LANGUAGE:    en
#+TAGS: noexport(n) MEETING(m) SUPERVISION(u) PRESENTATION(p) BLOG(b) TRAINING(n)
#+TAGS: INSPO(i) DIY(d) TEACHING(h) REPRES(R) REVIEW(v) GSoC(G) CONFERENCE(C)
#+TAGS: WRITING(w) BRAINDUMP(B) VISIT(V)
...
** 2024-11 novembre
*** 2024-11-01 vendredi
**** CCRMA: Gloria del Santo                                :PRESENTATION:
...
Differentiable FDN via frequency sampling. FIR approximation of IIR
...
***** FLAMO: Frequency-sampling Library for Audio-Module Optimization.n
