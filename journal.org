...
#+TAGS: [ HARDWARE : RPI(R) TEENSY(t) PTP(T) ]
...
- that there's more performance to be squeezed out of the managed
  switch.
...
but again, a histogram would give a clearer illustrationq. This setup
offers something on the order of magnitude described in
[cite:@schleusnerSubMicrosecond2024], with an interval of approx. -80
-- +60 ns; comparable with figure 9b from Schleusner et al. There's
also some asymmetry on display in that figure; hard to say at a glance
whether its their client/slave that runs fast, as it appears to be
with mine.
