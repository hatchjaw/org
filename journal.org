# -*- Mode: org -*-
# -*- coding: utf-8 -*-
#+STARTUP: content indent inlineimages logdrawer hidestars
#+TITLE:       Research Journal
#+AUTHOR:      Thomas Rushton
#+LANGUAGE:    en
#+TAGS: noexport(n) MEETING(m) SUPERVISION(u) PRESENTATION(p) SOUTENANCE(C)
#+TAGS: BLOG(b) TRAINING(n) INSPO(i) DIY(d) TEACHING(h) REPRES(R) REVIEW(v)
#+TAGS: GSoC(G) CONFERENCE(O) WRITING(w) BRAINDUMP(B) VISIT(V)
#+TAGS: [ PROGRAMMING : R(r) PYTHON(P) JULIA(j) FAUST(f) CPP(c) ]
#+TAGS: [ TOOLS : EMACS(e) ORGMODE(o) GIT(g) ZOTERO(z) LATEX(l) ]
#+TAGS: [ AUDIO : SYNTHESIS(s) PHYSMOD(y) SPATIALAUDIO(S) NETWORKEDAUDIO(N) DSP(D) DDSP(F) ]
...
- How much does a 32-port PTP switch cost? Less than a grand?
  + [[https://github.com/jclark/rpi-cm4-ptp-guide/blob/main/switches.md][Yes, some come in less expensive than that]].
...
Incidentally, stopping and restarting =ptp4l=, the router seems to be
able to re-attain a good sync, unlike the situation where a Teensy
acts as the clock authority. Why? Perhaps the =t41-ptp= clock authority
imlementation sets up some faulty state when potential clock
...
- RouterOS: Transport: l2-non-forwardable.
  + NB. in case of garbage (Teensy receiving messages from both the
    router /and/ the dongle), try l2-forwardable.
...
frequency. For the PTP timer, it's sort of like a resampler; instead
of incrementing 40 ns per cycle, every \(n\) cycles, we increment by
41 or 39 depending on the polarity of the adjustment. E.g.
...
- Get an abstract to [[https://www.i3da2025.org/][I3DA]] (Immersive and 3D Audio) by April 15th.
  ...
  + Anyway, the paper template can be found [[https://www.i3da2025.org/call-for-papers/#submit_paper][here]].
...
*** 2025-04-15 mardi
**** SMC Reviews, I3DA                                    :REVIEW:WRITING:
So one of my SMC papers was kind of racist (see the [[denote:20250405T153234][review]]). I'm quite
frustrated that it's taken until today to finish reviewing all
three,[fn:2025041501] but the problematic nature of the paper in
question introduced an additional burden. Now that I submit my 'hard
reject' (-3), I am able to see (which I wish I couldn't) that the
other reviewer to have submitted to-date gave the paper an 'accept'
(+2). I don't imagine I've heard the end of this one.
...
***** Footnotes
[fn:2025041501] Even with the help of [[https://notebooklm.google.com/][notebooklm]]... or maybe in spite
of that "help" since it appears to have been prompted to be, with
regard to uploaded papers, a condescending sycophant.

Entered on [2025-04-15 mar. 15:56]

*** 2025-04-16 mercredi
**** GSoC Proposals                                                 :GSoC:
***** Backend for MOJO --- Vijeta Priya
Engaged with us on Discord; proposal iterated a couple of times;
showed limited imagination, however (some of my suggestions wound up
in the proposal word-for-word), and doesn't identify a mentor from the
MOJO community. 26-page proposal; some LLM garbage. Since the
faustideas brief deals with performance, for once this is an
appropriate consideration.

There's some prototype code, and a "benchmarking report" [[https://drive.google.com/drive/folders/1Rw3A8mB6rGoRYQ9UlTAqkRMl6AeN7_JR][here]].

A 🤷‍♂️, I guess.

***** Developing Modular Synthesis using Widget Modulation --- Eslam Wageh Samer
Another prospective contributor who put a fair deal of effort into
making contact and asking for feedback. It's a good proposal (if
long/rambling), and it's accompanied by a working prototype, albeit
with pre-compiled modules.

Eslam clearly comes into this with no knowledge of Faust prior to a
couple of months ago, but he has shown great initiative. Tellingly, he
doesn't come from a modular synthesis background (nor anything
audio-related as far as his proposal indicates). Unlikely to be
accepted, furthermore, as Yann would have to mentor, and the view is
his time can't be spared for this.

Another 🤷‍♂️.

***** Faust LSP --- carn181 (Ryan Varghese)
Already accepted; Discord chat created.

Previously created a RISC-V [[https://github.com/carn181/tree-sitter-riscv][tree-sitter]] and basic [[https://github.com/carn181/riscv-lsp-vscode][LSP VS Code
extension]]. Also some basic [[https://github.com/carn181/syriac-input][Emacs]] [[https://github.com/carn181/garshuni-mlm][packages]]. Musician. Karl Hiner (whose
blog [[https://karlhiner.com/jupyter_notebooks/intro_to_digital_filters][slaps]]), creator of tree-sitter-faust, agreed to mentor.

That's a ✅, in any case.

***** Faust programming by examples --- kunxal (P Kunal Kumar)
I'm not aware of Kunal having reached out on Discord. Proposal has
some nice ideas, like gamifying /programming by examples/ and
permitting reverse input --- i.e. code-to-timeline. Not enough here,
beyond enthusiasm, to trouble my conscience. ❌

***** Faust programming by examples --- Siddharth Doshi
Siddharth was in touch for feedback, but is another one that took my
suggestions and incorporated them verbatim in his proposal. He did,
however, create a [[https://github.com/Siddhuuuu/Faust-Programming-by-Examples-Prototype][prototype]] that works well enough to demonstrate the
basic principle, and appears to understand the motivation behind the
project. Problem is this is a huge, open-ended, Elody-like affair, and
Yann probably can't be permitted to devote the time. 🤷‍♂️

***** faust2clap: Support for CLAP format --- Facundo Franchino
Decent proposal, aware of the motivation and related work. Undergrad,
Argentinian, studying in York. Fun [[https://facundofranchino.com/][website]]. Entrepreneurial,
self-promotional attitude; how much of this is hot air? 

We shall see. Stéphane is on board and found a co-mentor: ✅ is the
verdict.
***** HS-Faust: Haskell DSL for DSP via Faust Signal API --- Divit Mittal
What is this? A way to use Haskell as a language for interacting with
Faust's signal API? Repo [[https://github.com/DivitMittal/hs-faust][here]]. "This allows one to define Faust signal
processing graphs within Haskell and compile them using the Faust
infrastructure."

Frankly this is unhinged. I think this kid probably has talent, but
there's nothing in the proposal that tells me why he thinks it's a
good idea. ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- M Arslan Sherazi
First (well, second after Ryan I guess) of a number to propose on this
topic.

Insulting. For once I wish they'd used an LLM. ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- Navneet Verma
Three pages. Can you say "seamlessly integrates"? Topics: "Vision,
Robotics, Cloud, Graphics, Web, Machine Learning"... maybe they meant
things they've heard of. Anyway ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- Ayush Sonkar
"Seamless experience". No, that's unfair. This one's trash because
it's three pages with no description of what an LSP actually is.  ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- Abishek J Nair
Awareness of tree-sitter-faust; mentions the web IDE and its
shortcomings; went as far as to setup a skeleton [[https://github.com/hea7hen/faust-langserver-extension-prototype][repo]]. Not really a
complete proposal, however. ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- Bhupendra kumawat
Sensible keywords... but "seamless integration with VS Code" and
"seamlessly connects the LSP and the formatter". Three-pager, no
mentor, no contact, etc., etc. ❌

***** Integrated Faust Language Server and Formatting Extension for VS Code --- AditaMishra
Nearly the best of a bad sub-bunch... but with some (probably
LLM-generated) placeholder/template text left lying around:
#+begin_quote
5. Your Qualifications and Skills:
I possess a strong foundation in programming principles and have experience in the following
areas relevant to this project:
• Faust Programming: [Describe your experience with Faust, including any projects or
coursework you've undertaken. Highlight your understanding of its syntax and core
concepts.]
#+end_quote
Breaks my heart a little; this kid clearly tried to put at least some
things in their own words. ❌

***** Integration in Audiokinetic Wwise --- pmelaudev (Paschalis Melissas)
Audio middleware for game development. Masters level applicant, audio
background, /impenetrable/ [[https://github.com/pasquale90/mthesis][thesis]], but some decent looking projects
elsewhere. Acknowledges in his proposal that he ought to have said
hello on Discord. Honestly, this is strong; I think it could be a
goer. Lack of a mentor hurts it; am I prepared to step up? 🤷‍♂️

***** Integration in BELA --- Hsin Wen
Hsin said "Hello Sir, I would appreciate any feedback you could
provide on my proposal. Thank you!" I almost want to say yes on that
basis alone.

Anyway, Hsin doesn't have a Bela. Had to be prompted to describe Bela
and Faust and any sort of background to the project following a
context-free first draft. Honestly, looks like a good candidate, and,
being London-based, may have encountered Bela organically, as it
were. Can't see it working, or a mentor being found for it. ❌/🤷‍♂️

***** Integration in Surge --- Drishan Gogoi
No community engagement. Minimal background/motivation. LLM says:
emoji bulletpoints, "Seamless integration", "Testing &
Optimization". ❌

***** Intergration of Faust Web Audio Library into Cables.gl --- Sharvari Jadhav
It's been done. ❌

***** ofxFaust - Integration in openFrameworks --- Alex Hojin Park
This isn't bad; Alex has some audio background, made a couple of
merged PRs against the /of/ documentation site... but only names
Stéphane as prospective mentor. No real background/motivation
described. Pity. ❌/🤷‍♂️

***** PluginGuiMagic architecture (GRAME) --- Princy Patwa
Solid, honest. Not bad really. Just... /carré/. Better than most of
the other ❌'s but probably still in that camp.

***** Proposal for Faust Language Server and VS Code Extension Development --- Mohammed Ashar Khan
Little of substance here. ❌

***** Support for CLAP format --- Priyanshuwu
Quite good. Engaged with us during the proposal period, made a couple
of LLM-supported Faust PRs. Bit of a nuisance, tbh. Interest in audio
though; seems to understand what's at play. But Facundo has it, so ❌

Entered on [2025-04-16 mer. 10:17]
**** Inria PhD meeting with Julie Marchesi & Emma De Reboul      :MEETING:
- CSI: send scan of student card and CSI report to srh-lys@inria.fr
- Ask Solène about Inria integrity training.
- Consider talking to Mme Le Prince about a dentist appointment.

Entered on [2025-04-16 mer. 10:27]
*** 2025-04-17 jeudi
**** MIDI 2.0                                                :INSPO:AUDIO:
The long-promised dawn of MIDI 2.0 remains just round the corner as
ever. That being said (courtesy of the Linux-audio-dev mailing list),
improved support is indeed gathering pace on all the major
platforms. Courtesy of the Linux-audio-dev mailing list for
highlighting it, there's a little [[https://midi.org/latest-midi-2-0-developments-in-apple-google-linux-and-microsoft-operating-systems][article and video]] on midi.org.

There's [[https://developer.apple.com/documentation/coremidi/incorporating-midi-2-into-your-apps][stuff]] on MIDI 2 on the Apple developer site. On the Linux
side, there's chat about it being possible to easily set up embedded
devices as USB MIDI 2 devices.

Entered on [2025-04-17 jeu. 11:05]
**** Internet of Sounds Journal special issue              :INSPO:WRITING:
Via the internet-of-sounds network, I'm aware that there's to be a
special issue on "[[https://www.springeropen.com/collections/spios][Signal Processing for the Internet of Sounds]]" in the
[[https://asmp-eurasipjournals.springeropen.com/][EURASIP Journal on Audio, Speech, and Music Processing]]. Deadline is
June 11th, so it's very, /very/ unlikely.

That being said, relevant topics include parallelisation, spatial
audio for IoS, embedded audio, distributed audio, etc.

Gloria Dal Santo just had [[https://asmp-eurasipjournals.springeropen.com/articles/10.1186/s13636-025-00401-w][something]] published in that journal, fwiw.

Entered on [2025-04-17 jeu. 11:19]
**** Meeting: Benjamin
Peyrusse stuff:
- Benjamin to introduce dual numbers, but from more of a computational
  perspective...
  + so I could do numbers \to dual numbers \to dual numbers
    of dual numbers, i.e. higher-order derivatives.
  + Differentiable compositions
  + Higher-order derivatives
  + Examples: lowpass, impulse response... neural network.

Entered on [2025-04-17 jeu. 12:35]
*** 2025-04-18 vendredi
**** Latest on =intellij-faust=                           :INTELLIJ:FAUST:
Finally, finally got /run configurations/ working in their most basic
form. It's possible to summon the Faust compiler against a given input
file via a run configuration, just like one would with a Kotlin or C++
build, albeit in a stripped-down form.

As a reminder, I should replace file-based indexing with stub
indexing. [[https://github.com/JetBrains/Grammar-Kit/blob/master/HOWTO.md#35-stub-indices-support][Here]]'s some info on how to approach that via =grammar-kit=.

Entered on [2025-04-18 ven. 15:32]
  
  [[file:~/org/journal.org::*Faust Support for Intellij IDEs][Faust Support for Intellij IDEs]]
**** Delock ethernet dongle problems                            :HARDWARE:
Here I am trying to use the dongle, but =ethtool= says it has no
timestamping capability. This suggests the driver is missing or out of
date or something. I downloaded the latest version, but when I try to
=make= I find that the result of...
#+begin_src shell :results output :exports both
uname -r
#+end_src
...
... and the most recent directory in =/lib/modules=, don't match.
#+begin_src shell :results output :exports both
ls -l /lib/modules
#+end_src
...
#+RESULTS:
: total 8
: drwxr-xr-x 3 root root 4096  4 mars  09:37 6.6.65-1-MANJARO
: drwxr-xr-x 5 root root 4096 18 avril 16:52 6.6.85-2-MANJARO

(for the record, at the time of writing these return
=6.6.84-1-MANJARO= and =6.6.85-2-MANJARO= respectively.

Looks like a restart is required.

***** Post-restart: =modprobe=
Now, the kernel version number is a consistent =6.6.85-2-MANJARO=. I
forgot, however, to set ~ENABLE_PTP_FUNC~ to ~y~ in the
makefile. Uninstalling, cleaning, setting that variable, /then/
following the readme instructions properly, we have:
...
=modprobe= adds (or removes, with the =r= flag) the module to the
kernel without necessitating a restart.

Now we have:
#+begin_src shell :results output :exports both
ethtool -T enp85s0u2u3u1
#+end_src

#+RESULTS:
#+begin_example
Time stamping parameters for enp85s0u2u3u1:
Capabilities:
	hardware-transmit
	software-transmit
	hardware-receive
...
Entered on [2025-04-18 ven. 17:31]
  
  [[file:~/org/journal.org::*Delock 66045 LAN Adapter: driver installation & setup][Delock 66045 LAN Adapter: driver installation & setup]]

*** 2025-04-21 lundi
**** What lies +beneath+ ahead?                        :WRITING:BRAINDUMP:
***** Séminaire Emeraude: May 15th-17th
Presentation on DDSP to prepare; notes on recent meeting with Benjamin
[[*Meeting: Benjamin][here]].
***** CSI: May 27th
I'm to deliver documentation by May 13th. Instructions [[http://edinfomaths.universite-lyon.fr/en/thesis/thesis-follow][here]].
***** I3DA: June 15th
Notification of abstract acceptance will be made on April 30th. See
the [[https://www.i3da2025.org/][conference website]].
***** IS2/IEEE AWNIA: July 1st
Like I3DA, this is [[https://internetofsounds2025.ieee-is2.org/][one]] (well, [[https://internetofsounds2025.ieee-is2.org/workshops/3rd-ieee-international-workshop-networked-immersive-audio][two]], or three if you count the DDSP
[[https://internetofsounds2025.ieee-is2.org/workshops/3rd-ieee-international-workshop-networked-immersive-audio][workshop/tutorial]]) for which I need some actual results.
***** Bonus: SMC/AES-AIMLA?
Depends on acceptance. I guess a poster for SMC; I think a video for
AES (can't remember where I saw that... might've been another
conference). SMC annoncements are made on teh 28th of this month. AES
verdicts aren't released till June 16th.

Entered on [2025-04-21 lun. 17:21]
*** 2025-04-22 mardi
**** Chat with Yann                                 :INSPO:NETWORKEDAUDIO:
Yann asked me what I'm working on; I explained that I'm trying to get
back into the networked audio situation.

- He mentioned that Tanguy and Jean-Marie Gorce were trying to achieve
  something similar to what I'm working on a few years ago, but based
  on radio rather than ethernet.
  + But ethernet is ubiquitous; could one create a bidirectional,
    low-cost system based on radio? [cite:@lienhartImportance2003;
    @kiselevWHISPER2017] are radio-based, and I see Arduino radio
    TX/RX boards for a handful of € apiece...
- Wouldn't it be nice to daisy-chain the Teensies?
  + Yes, but then they'd have to be reading incoming data /and/
    passing it on, as transparently as possible. There are two ENET
    subsystems on the NXP IMXRT1062, but only one ethernet PHY on the
    Teensy board.
- If transmission were unidirectional, and all clients were reading
  from one /wire/, as it were, one could more-or-less take for granted
  that reception was synchronous. Then one could use buffer fill-level
  as a measure of drift.
  + But then you'd only have resolution of, at best, 1 sample (as far
    as I can see), and, in a packet-based transmission system, likely
    much coarser than that.

Worth reflecting on the above, nonetheless. And I really must get
around to asking related questions on the Teensy forum, in addition to
posting about my work to-date, and the supporting work by
[cite:@schleusnerSubMicrosecond2024].
  
Entered on [2025-04-22 mar. 18:40]
*** 2025-04-23 mercredi
**** PTP Setup; System Clock Sync                                    :PTP:
I'm really not sure at this stage whether I'm doing things right, but,
let's see, I have:
- =ptp4l= running
  #+begin_src shell :results output :exports both
sudo ptp4l -m -2 -i enp85s0u2u3u1
  #+end_src
- I did nothing to speak of with =pmc=
  #+begin_src shell :noeval
❯ sudo pmc -u -b 0 "GET CURRENT_DATA_SET"
sending: GET CURRENT_DATA_SET
        6c6e07.fffe.030595-0 seq 0 RESPONSE MANAGEMENT CURRENT_DATA_SET 
                stepsRemoved     0
                offsetFromMaster 0.0
                meanPathDelay    0.0
  #+end_src
which is to say the machine whose MAC ends 03:05:95 (i.e. the Delock
dongle), is the master clock right now.
- Attempting to set the system clock to match the PTP clock, =phc2sys=
  reports the following:
  #+begin_src shell :noeval
❯ sudo phc2sys -m -s enp85s0u2u3u1 -c CLOCK_REALTIME -w
phc2sys[63154.952]: CLOCK_REALTIME phc offset   3887514 s0 freq   +8525 delay 482982
phc2sys[63155.956]: CLOCK_REALTIME phc offset   3859769 s1 freq  -19195 delay 431388
phc2sys[63156.959]: CLOCK_REALTIME phc offset    104308 s2 freq  +85113 delay 532763
phc2sys[63157.963]: CLOCK_REALTIME phc offset     27877 s2 freq  +39975 delay 538869
phc2sys[63158.965]: CLOCK_REALTIME phc offset    -52894 s2 freq  -32433 delay 387954
phc2sys[63159.968]: CLOCK_REALTIME phc offset     37443 s2 freq  +42035 delay 492379
phc2sys[63160.971]: CLOCK_REALTIME phc offset    -13072 s2 freq   +2753 delay 390775
phc2sys[63161.974]: CLOCK_REALTIME phc offset     31370 s2 freq  +43274 delay 520288
phc2sys[63162.977]: CLOCK_REALTIME phc offset    -46635 s2 freq  -25320 delay 428014
phc2sys[63163.980]: CLOCK_REALTIME phc offset     38599 s2 freq  +45923 delay 460589
phc2sys[63164.983]: CLOCK_REALTIME phc offset      5585 s2 freq  +24489 delay 460682
...
  #+end_src
  but offset never converges (perhaps because we're trying to set a
  software clock?). In fact, it appears that the system clock isn't
  updated. Running
  #+begin_src shell :noeval
❯ sudo phc_ctl enp85s0u2u3u1 get
phc_ctl[63569.191]: clock time is 1745415069.946199320 or Wed Apr 23 15:31:09 2025
  #+end_src
  yields a time that's 37 seconds (the TAI-UTC offset) ahead of what's
  reported by the system (in the main window task bar clock for
  example).

***** pmc
It turns out that to get help on =pmc= commands, it's necessary to
send "help" as a command, e.g.

#+begin_src shell :noeval
❯ sudo pmc -u -b 0 -t 1 "help"
...
Entered on [2025-04-23 mer. 15:13]
**** Long-term Audio Sync                      :NETWORKEDAUDIO:TEENSY:PTP:
At the server side, when timestamping packets, reading from the PTP
clock --- in software --- isn't viable; under such an approach,
outgoing packets should be timestamped at the physical layer (just
like PTP packets) otherwise, while the average transmission interval
might be somewhat reliable, there will certainly be significant
deviations from that average. In any case, since the server's audio
clock isn't being conditioned like the clients' are, the result is
buffer overflow/underrun at the client side.

To test this, I tried PTP-timestamping the first packet transmitted,
then just adding 2666666 (ns) to the timestamp of each subsequent
packet. The result is that, while clients stay closely synced with
each other, they drift relative to the server. Since they all appear
to run faster than the server, they eventually underrun. This might
take a long time, so perhaps not all is lost; it's a problem
nonetheless.

I find myself considering, once again, the Teensy-USB-audio-device
approach; but how to get that to work at 48 kHz? It would also be nice
to decouple packet size from buffer size. I also need to change from
sample- to channel-interleaved format. There's no reason, per-se, that
packets should necessarily carry a power-of-two number of frames; like
AOO, packets should be arbitrarily fragmentable.

Entered on [2025-04-23 mer. 18:31]
*** 2025-04-24 jeudi
**** Romain B: Preparation for ARITH                        :PRESENTATION:
- Could give more of an intro; something more personable.
- Slow down a little.
- /Faithful rounding/ was a little unclear.
- Relevance domains? (slide 9)
- 14, could be clearer on significance/meaning of results.

***** Pronunciation/orthography
- "Approached" ("apprawched")
- "Array" (rhotic R)
- "Additionally" ("addishnly")
- "n equal three" -> "n equals three"
- "after the comma" -> "after the decimal point" (slide 12)
- "scientific writing" -> "scientific representation"
- "let me go step by step on the equation" -> "through the equation"
- 13, "factored out of the square root"
- "nineteen-hundred fifty-nine" -> "nineteen fifty-nine" (16)
- 17, "shit and add" 😅
- "naïve" -> "naive" (consistence)
- "...makes CORDIC only the practical solution" -> "the only practical
  solution"

Entered on [2025-04-24 jeu. 10:06]

*** 2025-04-25 vendredi
**** Audio Server Problems                           :NETWORKEDAUDIO:JUCE:
At least half the time, Ananas is throwing a segfault. As far as I can
recall, it didn't used to do this. It happens in
~AudioTransportSource::getNextAudioBlock()~ when the next block of the
~masterSource~ member is requested. Here's the typical trail of ~DBG~
messages:

#+begin_example
...
About to try to read from the FIFO
FIFO waiting for samples.
Checking readiness...
0 samples available
Copying from transport to buffer.

Process finished with exit code 139 (interrupted by signal 11:SIGSEGV)
#+end_example

And a run that was unafflicted by the segfault:
...
0 samples available
Copying from transport to buffer.
Passing buffer to Ananas server.
Writing to FIFO.
Notifying send thread.
Checking readiness...
128 samples available
FIFO done waiting. About to read from FIFO.
Sent the packet.
...
#+end_example

Potential causes:
- Something related to a recent system update? Manjaro/Linux kernel or
  library stuff?
- My ill-informed used of scoped locks, mutexes, or condition
  variables?

Infuriatingly, I didn't take notes at the time re. why I switched from
~juce::WaitableEvent~ to ~std::condition_variable~; something to do
with the former not triggering reliably? ~juce::WaitableEvent~ uses
~std::condition_variable~ under the hood; perhaps I should try
switching back?

Commenting out ~AnanasServer::getNextAudioBlock~ in ~MainComponent~
appears to prevent the segfault, which makes sense since it means that
the condition variable is sat there waiting forever.

***** Resolution!
With the caveat that I can't be 100% certain, I think it was a problem
with changing audio device setup. Basically the solution is to avoid
calling ~AudioTransportSource::prepareToPlay~ or
~AnanasServer::prepareToPlay~ until the expected number of frames per
block is as I expect it; otherwise I think the outgoing packet is
initialised, then later resized; meanwhile the audio thread is waiting
to write the original block size's worth of frames to the
buffer. Writing 512 frames to a buffer with a size of 128 can
/sometimes/ result in memory that's sufficiently mangled to cause a
segfault.

Entered on [2025-04-25 ven. 15:27]

*** 2025-04-27 dimanche
**** Audio packet format                           :ANANAS:NETWORKEDAUDIO:
To interlave or not to interleave?

Currently, Ananas /does/, which is to say that packets are of the
following format:
...
By way of comparison, NetJUCE just concatenated the data for each
channel, like so:
...
From the JUCE side, altering the format (from interleaved to not) is a
trivial case of switching from ~juce::AudioData::Interleaved~ to
~juce::AudioData::NonInterleaved~ when declaring the desired
~juce::AudioData::ConverterInstance~ (and altering the logic for
converting samples and writing them to a ~uint8_t*~ destination).

At the Teensy side, things are a little more complicated. Essentially,
the I^{2}S interface expects to receive interleaved audio data... and the
~AnanasServer~ implementation at that side receives an interleaved
stream of audio data, which it simply copies to the outgoing packet
as-is (~AudioServer::processAudio()~):
...
Is there necessarily any need to switch to non-interleaved at the
client side? I was thinking it would be a good idea, since that would
decouple the abstract ~AudioProcessor::processAudio()~ from the I^{2}S
interface, but I'm no longer sure. A Teensy is unlikely to act as the
audio server, even if I go down the USB audio device route (in which
case it would just, ideally, dictate audio rate to the desktop
machine).

Is there any advantage to interleaving when it comes to packet
fragmentation? It's probably no more than a design choice, but say
there are eight channels, and a packet holds a nominal 1500 bytes;
each sample is two bytes, each frame is 16 bytes; ignoring the header
for a sec:
...
(let* ((mtu 1500.0)
       ...
       (framesPerPacket (floor (/ mtu bytesPerFrame))))
  (format "%d frames per packet" framesPerPacket))
#+end_src

#+RESULTS:
: 93 frames per packet

Fine. Server-side, writing that many frames to a packet is possibly
marginally easier than calculating /how many channels can be written
to the packet at an arbitrary block size/, i.e. perhaps one keeps an
intermediate buffer, and a per-packet byte-countdown. Either way, I
think putting meaningful timestamps in arbitrarily-fragmented packets
is going to be a bit of a challenge.

Entered on [2025-04-27 dim. 16:21]
*** 2025-04-28 lundi
**** Time, time, time, time, time                                    :PTP:
Alright, =phc2sys= isn't going to be any help to me, I don't think.

To [[https://linuxptp.nwtime.org/documentation/phc2sys/][recap]]:

#+begin_quote
phc2sys is a program which synchronizes two or more clocks in the
system. Typically, it is used to synchronize the system clock to a PTP
hardware clock (PHC), which itself is synchronized by the ptp4l
program.
#+end_quote

My use-case is not the typical one, insofar as the Delock dongle, the
switch, and the Teensies are all counting from the Unix epoch when
they start up; this is to say that my "authoritative source of time"
(be it the dongle, the switch, or a Teensy) is running fifty-five
years slow.

Attempting to sync the system clock (=-c CLOCK_REALTIME=) to something
(=-s enp85s0u2u3u1=) in the very early seventies...

#+begin_src shell :noeval
sudo phc2sys -s enp85s0u2u3u1 -c CLOCK_REALTIME -w -m
#+end_src
...
#+begin_example
...
phc2sys[164740.710]: CLOCK_REALTIME phc offset -2656469781 s2 freq -100000000 delay 479445
phc2sys[164741.712]: CLOCK_REALTIME phc offset -2656513062 s2 freq -100000000 delay 423154
phc2sys[164742.716]: CLOCK_REALTIME phc offset -2656453903 s2 freq -100000000 delay 581436
phc2sys[164743.720]: clockcheck: clock jumped backward or running slower than expected!
phc2sys[164743.720]: CLOCK_REALTIME phc offset -5587814982 s0 freq -100000000 delay 468789
phc2sys[164744.724]: CLOCK_REALTIME phc offset -5587767039 s2 freq -99826137 delay 594245
phc2sys[164745.727]: CLOCK_REALTIME phc offset -5587935212 s2 freq -100000000 delay 516522
...
#+end_example

I.e. a lock is never achieved.

If I attempt to use the system clock as the source:

#+begin_src shell :noeval
sudo phc2sys -s CLOCK_REALTIME -c enp85s0u2u3u1 -w -m
#+end_src
...
#+begin_example
...
phc2sys[163660.749]: enp85s0u2u3u1 sys offset -891786520 s2 freq -100000000 delay 328641
phc2sys[163661.752]: enp85s0u2u3u1 sys offset -669158461 s2 freq -100000000 delay 357971
phc2sys[163662.755]: enp85s0u2u3u1 sys offset -446168085 s2 freq -100000000 delay 430705
phc2sys[163663.758]: enp85s0u2u3u1 sys offset -223276487 s2 freq -100000000 delay 434661
phc2sys[163664.761]: enp85s0u2u3u1 sys offset   -169617 s2 freq +83180527 delay 351178
phc2sys[163665.764]: enp85s0u2u3u1 sys offset  18768645 s2 freq +100000000 delay 321847
phc2sys[163666.766]: enp85s0u2u3u1 sys offset  18794683 s2 freq +100000000 delay 339190
...
phc2sys[163678.799]: enp85s0u2u3u1 sys offset  18835115 s2 freq +100000000 delay 550386
phc2sys[163679.802]: enp85s0u2u3u1 sys offset  18930552 s2 freq +100000000 delay 369837
phc2sys[163680.805]: enp85s0u2u3u1 sys offset -3564295263 s2 freq -100000000 delay 442019
phc2sys[163681.807]: enp85s0u2u3u1 sys offset -3341692909 s2 freq -100000000 delay 312034
phc2sys[163682.810]: enp85s0u2u3u1 sys offset -3119159769 s2 freq -100000000 delay 449463
phc2sys[163683.812]: enp85s0u2u3u1 sys offset -2896268347 s2 freq -100000000 delay 317654
phc2sys[163684.815]: enp85s0u2u3u1 sys offset -2673683409 s2 freq -100000000 delay 438819
...
#+end_example

which is to say that attempting to condition the PTP clock to the
system clock results in dramatic frequency-oscillations (or maybe
numerical overflow somewhere). Sort of makes sense, since the system
clock is probably really low-res...

Thing is I don't really want to condition the PTP clock this way; I
guess it'd be nice to have a *one-time update* that brings my local
PTP environment screaming into the 2020's, but after that it doesn't
matter as long as the network follows one predictable, reliable time
source.

So, how to proceed?

***** This afternoon's attempts

The closest I've come to synchronising the PTP and system clocks is as
follows:

- Run a Teensy as clock authority (with highest priority in the network).
  + Using the following script, I can generate a number of seconds
    since the Unix epoch to a time of my choosing (e.g. today at
    17:03:00)
  ...
  (let ((now (decode-time)))
    (setcar now 0)
    (setcar (cdr now) 24)
    (setcar (cdr (cdr now)) 17)
    (format "Seconds since the Unix epoch: %d" (time-convert (encode-time now) 'integer)))
    #+end_src

    #+RESULTS:
    : Seconds since the Unix epoch: 1745940240

    which I can paste into the ~onLinkState~ callback in the clock
    authority's ~setup()~ function and set the inital 1588 timer
    value:
    
    #+begin_src c++
  if (state) {
    ptp.begin();
    const timespec t{1745852580, 0};
    qindesign::network::EthernetIEEE1588.writeTimer(t);
    //...
    #+end_src
    
- Sync the Delock adapter to the Teensy via the switch.
  + L2(-non-forwardable), E2E.
    
- Run =ptp4l= with a modified version of gPTP config
...
delay_mechanism	E2E
  ...
  #+begin_src shell :noeval
sudo ptp4l -m -f ~/devel/linuxptp/configs/gPTPE2E.cfg -i enp85s0u2u3u1
  #+end_src
  
  Default priority is [128, 128], so this helps get the dongle out of
  the way, as it were. Otherwise I might use
  
  #+begin_src shell :noeval
sudo ptp4l -m -2 -i enp85s0u2u3u1
  #+end_src
  
  + =ptp4l= (eventually) syncs to the Teensy; the switch does so much
    more rapidly.
    
- I've experimented with using =pmc= as per the [[https://tsn.readthedocs.io/timesync.html#synchronizing-the-system-clock][Linux TSN docs]]:
...
sudo pmc -u -b 0 -t 1 "SET GRANDMASTER_SETTINGS_NP clockClass 248 \
  ...
  but to no appreciable avail.
  
- I can check the time as reported by the system (=CLOCK_REALTIME=)
  and the PTP clock (=/dev/ptpN= or =enpXX...=) via
...
sudo phc_ctl /dev/ptp2 get
  ...
  and likely find that they're at least a few seconds apart.
  
- Finally, I try =phc2sys=, e.g.
  
  #+begin_src shell :noeval
sudo phc2sys -s enp85s0u2u3u1 -c CLOCK_REALTIME -w -m
  #+end_src
  
  or the equivalent automatic config:
  
  #+begin_src shell :noeval
sudo phc2sys -a -r -m
  #+end_src
  
  but what I tend to see is a failure to converge:
  
  #+begin_example
phc2sys[178495.418]: reconfiguring after port state change
phc2sys[178495.419]: selecting enp85s0u2u3u1 as domain source clock
phc2sys[178495.419]: selecting CLOCK_REALTIME for synchronization
phc2sys[178495.422]: CLOCK_REALTIME phc offset 41397366730 s0 freq  -17664 delay 448442
phc2sys[178496.424]: CLOCK_REALTIME phc offset 41397344224 s1 freq  -40120 delay 365278
phc2sys[178497.427]: CLOCK_REALTIME phc offset 41389124376 s2 freq +100000000 delay 518762
... [offset steadly decreases...]
phc2sys[178528.498]: CLOCK_REALTIME phc offset 37937325908 s2 freq +100000000 delay 366364
phc2sys[178529.500]: clockcheck: clock jumped forward or running faster than expected!
phc2sys[178529.500]: CLOCK_REALTIME phc offset 41295523063 s0 freq +100000000 delay 375098
phc2sys[178530.503]: CLOCK_REALTIME phc offset 41184152555 s2 freq -100000000 delay 404077
phc2sys[178531.506]: CLOCK_REALTIME phc offset 41275063813 s2 freq +100000000 delay 380758
  #+end_example
  
(Interestingly, I've got music playing in a web browser and it stutters
for a few seconds when =phc2sys= starts.)

***** Resources
There's a nice [[https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/6/html/deployment_guide/ch-configuring_ptp_using_ptp4l][guide to PTP in general]] on the Red Hat docs
site. Perhaps poring over that will yield further leads.

Entered on [2025-04-28 lun. 13:50]
**** SMC Paper Accepted                                          :WRITING:
As usual I'm too nervous to read the feedback in detail, but it
received a 3, a 2, and another 2, which is rather good. I feel quite
relieved.

The reviewers found a couple of typos! (And one cast shade on my
title!) Camera-ready version due on June 2nd.

***** Addendum
My I3DA abstract was accepted too. By way of a reminder, the deadline
is June 15th.

Entered on [2025-04-28 lun. 18:58]

*** 2025-04-29 mardi
**** NTP was getting in the way                                      :PTP:
=phc2sys= entries to the effect /clockcheck: clock jumped forward or
running faster than expected!/ were because PTP and NTP were fighting
over the system clock (nothing to do with numerical wraparound or
anything like that). Per the TSN [[https://tsn.readthedocs.io/timesync.html#confirm-ntp-is-not-running][instructions]], it's necessary to make
sure that NTP isn't running:

#+begin_src shell :noeval
timedatectl set-ntp false
#+end_src

/Probably/ worthwhile resetting that to ~true~ when not running PTP.

Anyway, now =phc2sys= produces the following manner of output:

#+begin_example
phc2sys[202328.830]: reconfiguring after port state change
phc2sys[202328.830]: selecting enp85s0u2u3u1 as domain source clock
phc2sys[202328.830]: selecting CLOCK_REALTIME for synchronization
phc2sys[202328.832]: CLOCK_REALTIME phc offset   1300515 s0 freq  -35188 delay 333124
phc2sys[202329.834]: CLOCK_REALTIME phc offset   1368012 s1 freq  +32180 delay 363422
phc2sys[202330.837]: CLOCK_REALTIME phc offset      7414 s2 freq  +39594 delay 438018
phc2sys[202331.840]: CLOCK_REALTIME phc offset    -12433 s2 freq  +21971 delay 473469
phc2sys[202332.843]: CLOCK_REALTIME phc offset    -58924 s2 freq  -28250 delay 409873
phc2sys[202333.845]: CLOCK_REALTIME phc offset    -38587 s2 freq  -25590 delay 330397
phc2sys[202334.847]: CLOCK_REALTIME phc offset     18334 s2 freq  +19755 delay 364852
phc2sys[202335.849]: CLOCK_REALTIME phc offset     -7300 s2 freq    -379 delay 365280
phc2sys[202336.851]: CLOCK_REALTIME phc offset      -556 s2 freq   +4175 delay 338046
phc2sys[202337.854]: CLOCK_REALTIME phc offset     72679 s2 freq  +77243 delay 445849
phc2sys[202338.857]: CLOCK_REALTIME phc offset     22188 s2 freq  +48556 delay 501844
phc2sys[202339.860]: CLOCK_REALTIME phc offset    -10117 s2 freq  +22907 delay 529120
phc2sys[202340.863]: CLOCK_REALTIME phc offset    -88281 s2 freq  -58292 delay 377027
phc2sys[202341.865]: CLOCK_REALTIME phc offset    -25725 s2 freq  -22220 delay 360119
...
#+end_example

but it doesn't appear to converge to anything closer than something on
the order of \pm{}tens of microseconds. Perhaps because the system clock
for my particular machine is really bad; perhaps only adjustable to
microsecond resolution.

Anyway, bloody hell, =ptp4l= takes a long time (minutes) to converge
if I, say, restart the clock-authority Teensy with a new starting 1588
timer value. Now, there's a chance that I simply haven't configured
=ptp4l= properly to handle large jumps, but my sense is that the PTP
standard (or =ptp4l= in particular) isn't designed for precisely this
manner of time-based skulduggery, i.e. small adjustments fine, large
ones not (/except/ on startup --- goes for both =ptp4l= and
=phc2sys=); it runs with accurate time in mind, not with synchronous
audio as a priority. So maybe I need to tack away from that approach.

Anyway, the driver for the Delock interface (or perhaps for the Dell
dock) isn't well-enough written to survive a sleep/wake cycle. Ffs.

Entered on [2025-04-29 mar. 16:01]
*** 2025-04-30 mercredi
**** Teensy: USB audio device and PTP clock authority :NETWORKEDAUDIO:TEENSY:PTP:
I figured I'd give it another go, at 44.1 kHz. For a while I was
wondering /why the bloody hell is the clients' read position updating
every second?/ and then I remembered that 128/44,100 into 1 isn't a nice
number.
...
(let ((nsps 1e9)
  ...
  (format "Nanoseconds per buffer: %f\nBuffers per second: %f"
        (* nsps (/ bufferFrames samplingRate))
        (/ samplingRate bufferFrames)))
#+end_src

#+RESULTS:
: Nanoseconds per buffer: 2902494.331066
: Buffers per second: 344.531250

Unlike 128/48000, for which the result is at least /real/
...
#+RESULTS:
: Nanoseconds per buffer: 2666666.666667
: Buffers per second: 375.000000

and, at the default buffer size, we have a whole number of buffers per
second, which is why starting up the audio subsystem /on the PTP timer
interrupt/ works.

Anyway, running at 44.1 kHz, with one Teensy as a USB audio device and
PTP clock authority (for the other Teensies, the switch, and the
computer, the latter via =ptp4l=), it's possible to achieve a sync if
clients start up /at the same time/ i.e. on the same per-second PTP
timer interrupt (probably after calling =reboot.sh=, which results in
closely-aligned startup). After a long time running, a given client
reports the following:

#+begin_example
IP: 192.168.48.171 | MAC: 04:E9:E5:14:30:AB | SN: 13231790
Clock Subscriber - Frames/Fs: 128/44100 (44099.7149585422012024)
PLL4 DIV: 28, NUM: 223817573, DENOM: 1000000000; SAI1 PRED: 3, PODF: 20
Packets received: 1430256 Average reception interval: 2.902492e+06 ns
Read index: 22, Write index: 6, Num packets available: 34
Packet offset: -823109 ns, Sample offset: -36
#+end_example

That /Num packets available/ figure of 34 (occasionally 33) has been
steady throughout the run, and the /Average reception interval/ is
close to the /Nanoseconds per buffer/ figure above. This is in part
thanks to calling

#+begin_src c++
setup.outputDeviceName = "Teensy Audio, USB Audio; Front output / input";
#+end_src

in the Ananas server's ~MainComponent~ constructor, i.e. using the
Teensy as Ananas's output audio device (the success of which seems to
hinge on /not/ using the Teensy as the output device for the system as
a whole), and

#+begin_src c++
prevTime += 2902494; // 1e9 * 128/44100
header.timestamp = prevTime;
#+end_src

in ~AnanasPacket::writeHeader()~, manually bumping the packet
timestamp each time by the expected number of nanoseconds.

In short (and subject to finding a way to ditch =ptp4l=) I think this
approach could work, but of course we'll have to switch to 48 kHz. I
made a start on creating some modified versions of Teensy's
=usb_audio.h=, =usb_desc.h=, etc., but due to the way the Teensy
toolchain is compiled, this isn't a goer. In =cores/teensy4/Makefile=
we have
...
Those hardcoded instances of ~44100~ look awfully suspect.

I [[https://github.com/hatchjaw/teensy-cores][forked]] Paul Stoffregen's Teensy cores repository, and will
endeavour to do one or more of the following:
- hard-code it to support 48 kHz.
- make it so that ~AUDIO_SAMPLE_RATE_EXACT~ is used instead.
  + this would have ramifications for the Teensy audio library, which
    is managed via a separate [[https://github.com/PaulStoffregen/Audio][repository]].
- create a stripped-down version of the core libraries with Ananas in
  mind.
  + perhaps extending my HAL approach to the whole thing.
  + that sounds like way too much work; I should just rewrite it in
    Rust.

Entered on [2025-04-30 mer. 17:03]
** 2025-05 mai
*** 2025-05-04 dimanche
**** 48 kHz USB Audio with the Teensy              :NETWORKEDAUDIO:TEENSY:
(Following up on my previously-stated aims) I took the following
approach instead:

- Copy the bare essentials from
  =~/.platformio/packages/framework-arduinoteensy= to
  =audiosync/framework-arduinoteensy=; specifically, =cores/teensy4=,
  =package.json= (and other metadata), and a selection of libraries
  (Audio, Wire, SerialFlash, etc.).
...
...
  #+end_example
- Replace hard-coded references to 44100, etc. with
  ~AUDIO_SAMPLE_RATE_EXACT~.

I also changed the ~PRODUCT_NAME~ (and ~PRODUCT_NAME_LEN~) macro in
=usb_desc.h= to 'Teensy Ananas'.

My computer could see the available USB audio device, and (using the
only helpful command line utility I could find) I was able to confirm
that Teensy reported its sampling rate capability as 48 kHz.
...
#+RESULTS:
: 57	alsa_output.pci-0000_00_1f.3.analog-stereo	PipeWire	s32le 2ch 48000Hz	SUSPENDED
: 785	alsa_output.usb-Generic_USB_Audio_200901010001-00.HiFi__Line__sink	PipeWire	s24le 2ch 48000Hz	SUSPENDED
: 786	alsa_output.usb-Generic_USB_Audio_200901010001-00.HiFi__Headphones__sink	PipeWire	s24le 2ch 48000Hz	RUNNING
: 823	alsa_output.usb-Teensyduino_Teensy_Ananas_13231990-01.analog-stereo	PipeWire	s16le 2ch 48000Hz	SUSPENDED
...
***** Slow transmission
Audio output at Teensy's side was... slow, and discontinuous,
however. And a PTP client reported an average packet reception
interval of 2,850,000 ns. There are some troubling magic numbers in
=usb_audio.cpp=, but in the end what sorted out that problem was
bumping ~AUDIO_TX_SIZE~ and ~AUDIO_RX_SIZE~ in =usb_desc.h= from 180
to 200 (as a guess). That 180 appears to suggest that Teensy operates
as a low-speed device, sending (non-micro)frames on a per-millisecond
basis:

#+begin_src emacs-lisp
(let* ((fs 44100)
       (framesInOneMs (/ fs 1000))
       (numChannels 2)
       (bytesPerSample 2)
       (bytesPerUSBFrame (* bytesPerSample numChannels framesInOneMs)))
  (format "Bytes per USB frame: %f" bytesPerUSBFrame))
#+end_src

#+RESULTS:
: Bytes per USB frame: 176.000000

And the equivalent for 48 kHz?
...
So 200 bytes (which is an upper bound; a given frame may not
necessarily be full) was a decent guess. I think I'll leave the
headroom as it is.

***** USB/PTP drift
I was seeing a steady drift in the "Packet offset" (and "Sample
offset") reported by the client. It emerged that this was actually due
to my taking a shortcut in bumping the outgoing header timestamp. The
packet interval at 128 frames is \(2,666,666.\dot{6}\) ns, and I
wasn't taking account of those two-thirds of a nanosecond after the
decimal point. Much better now.

***** Still drifting somehow?
I witnessed some troubling decreases in the clients' "Num packets
available" values, e.g. a run starting with 37 packets available and
this decreasing to nothing, i.e. the buffer underrunning, suggesting
slow transmission from the computer to the USB-Audio Teensy. I decided
that this might be related to the magic numbers in =usb_audio.cpp=;
indeed, decreasing the ~target~ values toward the top of
~usb_audio_transmit_callback()~ and increasing the
~feedback_accumulator~ bump in ~update()~, introduced when the "PC
[is] sending too slow", from 3500 to 3800 (approximately proportional
to the sampling rate increase). Anecdotally, those changes appear to
have had a positive impact.

Entered on [2025-05-04 dim. 14:30]

*** 2025-05-06 mardi
**** CSI Progress Report                                 :WRITING:ORGMODE:
I'm working on the documentation (due May 13th) for my upcoming CSI
(May 27th, 8 a.m.). Interesting as it is to see how far I've come with
Org Mode (I hadn't finished the /Reproducible Research/ MOOC this time
last year), for LaTeX export, I'm troubled by the restrictions
stipulated by the =classicthesis/arsclassica= template (based on
=scrarctl=), specifically regarding typefaces --- the sans serif font
it uses for headings is ghastly. More generally, I'm troubled by the
fact that I've yet to devote the time to developing a consistent style
across my CV, reports, and presentations (though incorporating the
font [[https://fonts.adobe.com/fonts/minion][Minion]] into the latter could help).

This being so, I flirted momentarily with [[https://typst.app/][typst]], installing the
relevant [[https://github.com/jmpunkt/ox-typst][Org Mode exporter]]. Immediate impressions: /gosh, it compiles
quickly, and looks nice/; later: /Why doesn't my PdF have a title? Oh,
the Org title only winds up as metadata by default?/. I don't know why
I imagined otherwise, but, just like LaTeX, one needs to go
[[https://typst.app/universe][template-hunting]]. A point in favour of typst: works with .bib
files. Against: not possible to have your .bib file(s) outside of the
project root; possible to change the root at the command line
(e.g. =--root ../..=); =ox-typst= sets =--root= (which can't be
specified more than once), and doesn't expose a way to override it;
only sensible workaround I can think of is
~org-cite-global-bibliography: nil~ (in Local Variables),
~#+bibliography: my.bib~, and a symlink to the out-of-dir bib
file. Another point against: typst uses its own maths syntax; Org Mode
is rather LaTeX-centric when it comes to maths; there are a couple of
[[https://github.com/jmpunkt/ox-typst?tab=readme-ov-file#math][options]] when it comes to adapting to this situation.

***** Resources
- [[https://badnoise.net/TypeRip/][TypeRip]], for downloading Adobe fonts (in .ttf format).

Entered on [2025-05-06 mar. 14:32]

*** 2025-05-17 samedi
**** Peyrusse: Romain M.                                    :PRESENTATION:
- Maxime P. defended thesis; recently added companion website for
  experiment reproduction.
  + Created /Polyphonics/ startup with support of Inria Startup
    Incubator.
  + Working with /Music Unit/ (Paris) on a binaural implementation.
  + New Syfala board under development.
- Ethernet FPGA transmission stabilised in Syfala.
  + /Space Bar/ (second-iteration DIY FPGA WFS device) deployed at
    Inria showroom in Grenoble.
- Apparently I've released the "most precise distributed spatial audio
  system ever".
- Remi Jeunehomme had a paper accepted at SMC on his intership work.
- FPGA ambisonics dome, based on the same PCB as the /Space Bar/,
  working with a decoder based on /Ambitools/ (Faust).
  + Created by PIR students + Pierre.
- +PLASMA+ MIRAGES associate research team; Emeraude + CCRMA.
  + €10k/year for 3 years.
  + Modeling & Immersive Rendering of Archaeoacoustics in Grotto
    Environments.
  + Stanford does modeling; Emeraude does rendering.
- Outlook: AI & embedded sound systems
  + Remi Guillote internship; AI for reverb modeling.
  + Audio processing on NPU (N for /neural/) [[https://www.st.com/en/microcontrollers-microprocessors/stm32n6-series.html][STM32N6]] ([[https://www.st.com/en/evaluation-tools/stm32n6570-dk.html][demo board]]).
  + ERC proposal.
  + Syfala goes ASIC (via [[https://tinytapeout.com/][tinytapeout]]).
  
Entered on [2025-05-17 sam. 09:07]
*** 2025-05-20 mardi
**** SMC paper revisions                                         :WRITING:
***** Reviewer 1
Score: 3 (strong accept)
****** DONE Replace "Euro" with "euro" in table 1 caption
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:30]
:END:
****** CANCELLED Page 5, line 34: "an FPGA" \to "a FPGA"
:LOGBOOK:
- State "CANCELLED"  from "TODO"       [2025-05-20 mar. 17:30]
:END:
It turns out I misspelled FPGA ("FGPA"). With that fixed, the
convention appears to be "an" (see most of the uses in the [[https://en.wikipedia.org/wiki/Field-programmable_gate_array][Wikipedia
article]]).
****** DONE Add suggested references
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-22 jeu. 17:22] \\
  I included a reference to 'GNSS-based Sound Card Synchronization'.
:END:
They are useful papers, one of which (an Alexander Carôt number)
actually uses an NXP device. I'm a little disappointed I didn't find
them myself.
***** Reviewer 2
Score: 2 (accept)
****** DONE Maybe remove "All Together Now" from the title
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-21 mer. 17:06] \\
  I decided to add a few words in the conclusion including the phrase
  /all together/.
:END:
Either get rid, or mention these words in the text (without it coming
across as forced or pretentious).
****** DONE Enhance description of experimental setup
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-21 mer. 10:45]
:END:
#+begin_quote
distance between the various MCU and the switch, resolution of the
analyzer, etc. It should be reproducible by others.
#+end_quote
****** DONE P. 1, l. 57: our approach
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:51]
:END:
****** DONE P. 1, l. 60: in Section 5,
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:49]
:END:
****** DONE P. 1, l. 65: well established
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:48]
:END:
****** DONE P. 2, l. 26: power consumption \to power requirements
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:45]
:END:
****** DONE P. 3, l. 69: \(\hat{f}_{s}\), such that
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:42]
:END:
****** DONE P. 4, l. 74: are are
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:45]
:END:
***** Reviewer 3
Score: 2 (accept)
#+begin_quote
[I]'m not entirely sure, why in the sampling-rate adjusted experiment
(as seen in Fig.3b) the clients still have a general drift (albeit
small). shouldn't they perform a "random walk" centered around the
target samplerate instead?
#+end_quote
****** DONE Equation (1): change units for \(a\) from ns to s
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-21 mer. 10:18]
:END:
Hm, this one is slightly awkward. Suggestion followed
nonetheless.
****** DONE "availaibility" \to "availability"
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-05-20 mar. 17:40]
:END:

Entered on [2025-05-20 mar. 17:26]
*** 2025-05-21 mercredi
**** PIR: Faust Widget Modulation                           :PRESENTATION:
- History of analog synths; Moog, Eurorack, etc.
- Widget modulation in Faust.
- The group created "Source modules" and "Transformation modules"...
- Attempted to recreate the sound of the TB-303, 808 kick, etc.

Sadly there was too much that I couldn't understand (slides & talk
given in French).

Entered on [2025-05-21 mer. 14:06]
**** PIR: Algorithms for an FPGA-Based Spatial Audio Dome   :PRESENTATION:
- Goal: create a low-cost 3D immersive sound experience using a
  loudspeker dome and an FPGA.
- Use Faust & Syfala.
- Implement ambisonics; test the encoder/decoder.
- Ambisonics
  + Models a soundfield in a spherical zone delimited by loudspeakers.
  + Spherical harmonics.
  + Sonud field is a sum of spherical spatial functions.
  + Order is improtant; higher order, greater detail.
  + For each order, \(2n + 1\) extra degrees (output channels) needed.
  + Virtual sound sources... plane waves... multiple sources, etc.
  + Fractional orders possible, apparetly; order 3.5 more detailed
    than order 3 (but less than 4) --- how many speakers needed?
  + Encorder/decoder adaptable to the number of speakers.
- FPGA
  + Reprogrammable processor.
  + Contains a great number of logic gates...
  + Optimised to specific task.
  + Applications (came after "why use...") doesn't need a whole
    slide...
    * High-frequency trading, Aerospace, Embedded audio.
  + Why use one for the dome?
    * Ultra low-latency.
    * Parallelisation of computation.
    * Hardware connectivity...
    * Reconfigurable/reprogrammable.
    * Chepaer, more flexible than an an ASIC.
- dsp chain
  + Soruce audio \to encoder (PC) \to ethernet \to decoder (FPGA)
    * What's going on with the text/maths to the right of slide 19?
      Too small, dim, illegible.
  + Used Jack to patch an audio source to the encoder, which piped
    signals to the Syfala Jack client.
- Code/maths
  + Sound and angular position (azimuth/elevation) \to encoder \to vector
    of ambisonics channels.
    * Fix those spelling error indicators in your code...
  + Ambisonics channels \to decoder \to output signals.
  + Code examples rather pointless; too small; nothing to lead the
    eye.

Entered on [2025-05-21 mer. 15:03]
*** 2025-05-23 vendredi
**** Meeting: Romain                                 :MEETING:SUPERVISION:
- Keep SMC paper paragraph on GPS stuff.
- Don't necessarily do everything the reviewers say.
- SMC arrangements made.
- Got to move the speakers out of Humanities room 210.
  + for JIMLAC, but also because INSA is going to refurbish the
    building.
  + Romain wants to move them just before JIMLAC; week of June 16th.
  + Special event, midweek, ICCART; they need an 8-channel setup, but
    we also need some to give out.
  + Use the atelier after SMC for system tests. I'm going need access
    to CITI over the summer.
  + For I3DA, just see if I can find out whether the RAM chips are
    viable.
  + I^{2}S tutorial --- intro to Faust; give short project ideas; 90 mins
    is probably too short, so the three hour slot is the one to aim
    for. Oh, I actually don't need to have the materials ready till
    October. People maybe don't actually want to think, so spoonfeed
    them.
  + Maybe CCRMA won't work out; hard to say whether Stanford might
    follow Harvard's lead (this morning's news) and ban/reduce intake
    of foreign students.
  + École Centrale for testing, maybe? Romain to ask Pierre Lecomte.

Entered on [2025-05-23 ven. 09:56]
**** Inria's Scientific Subscriptions                         :LITERATURE:
Inria provides access to a large [[https://numin.inria.fr/portal/g/:spaces:71lwaa/base_de_connaissance/notes/3383?translation=en][collection]] of paywalled titles;
JSTOR, IEEE Xplore, etc. One option for accessing these is to add a
system-wide proxy:

Réseau \to Serveur mandataire \to https://ist-proxy.inria.fr/pac/proxy.pac

This poses the annoyance of browsers (and Zotero) requesting basic
authentication for /any/ (e.g.) IEEE URL. The alternative is a browser
extension, [[https://clickandread.inist.fr/][Click & Read]], which, as of right now, I'm trying instead.
***** Resources
- "[[https://numin.inria.fr/portal/g/:spaces:71lwaa/base_de_connaissance/notes/3752?translation=en][Access scientific subscriptions]]" documentation on Numin.

Entered on [2025-05-23 ven. 10:22]
*** 2025-05-27 mardi
**** CSI Year 2
This went fine. I spoke for a little too long (an extra five minutes
or so), and was my usual discursive self when responding to
questions. In any case, no worries. As Leo pointed out, however, I
should've introduced my thesis topic from first principles rather than
diving straight into a description of a year wrestling with hardware.

- Leo: MAKE A PLAN
  + Now's the time to be thinking about how to spend the rest of the
    second half; realistic goals, that sort of thing.
- Jens: similar stuff; it's impossible to do/implement everything, so
  be selective.
  + It /is/ possible to evaluate the technical side of the work and have
    that be a scientific contribution.
  + But what about evaluating the /consequences of drift/? What sort of
    discrepancies are perceptible? How long do they take to arise, etc.?
  + In the absence (or with question marks over availability at least)
    of a space and equipment with which to test setups based on my
    system (or perhaps as a complement) why not use binaural
    reproduction? HRTFs and artificial, simulated drift...

Jens contacted me after the meeting wrt. the above; he's keen to help
if I'm interested.

Entered on [2025-05-27 mar. 16:28]
*** 2025-05-28 mercredi
**** Comparing PDFs                                        :WRITING:TOOLS:
Preparing the camera-ready version of my SMC paper, I decided I needed
to be sure that I hadn't make any content changes subsequent to sharing
rev. 2-1 with Romain & Tanguy, i.e. I needed to compare =smc-2025.pdf=
and =smc-2025_rev2-1.pdf=. I tried =M-x diff=, but that just told me that
/these binary files differ/, not in what way (could just be metadata for
example). Anyway, I found the open-source [[https://vslavik.github.io/diff-pdf/][diff-pdf]], which works rather
nicely.
...
The above opens a PDF viewer with differences highlighted (no metadata
view as far as I can tell though).

Entered on [2025-05-28 mer. 16:06]
*** 2025-05-30 vendredi
**** Teensy: External Memory                                      :TEENSY:
Maxime was kind enough to show me how to attach a [[https://www.pjrc.com/store/psram.html][PSRAM chip]] to the
Teensy. Key takeaways:
- tin the pads with a thin layer of solder; remove any excess.
- align the chip, and attach one corner pin with a tiny bit of
  additional solder.
- one leg soldered anchors the chip; the rest should follow relatively
  easily with a small amount of solder.
- avoid bridging the pads; not the end of the world to have to remove
  some solder.

There's a [[https://github.com/PaulStoffregen/teensy41_psram_memtest/blob/master/teensy41_psram_memtest.ino][memory test program]], and, sure enough, with one chip
attached on one Teensy, it produces the expected output:
...
It runs 57 tests in total, each of which iterates over the entire 8 MB
memory twice. So, give or take, that's

#+begin_src emacs-lisp
(let ((numTests 57)
      (numIter 2)
      (memSizeBytes (* 8 1024 1024))
      (testDuration 36.43))
  (format "%f byte/sec"
          (/ (* numTests numIter memSizeBytes)
             testDuration)))
#+end_src

#+RESULTS:
: 26250379.138073 byte/sec

Mono, 16-bit, 48 kHz wav has a data rate of

#+begin_src emacs-lisp
(let ((bytesPerSample 2)
      (fs 48000))
  (format "%f byte/sec" (* bytesPerSample fs)))
#+end_src

#+RESULTS:
: 96000.000000 byte/sec

Now imagine we want to convolve with an impulse response; computing
each output sample is going to require access to (and computation
with) a number of samples equal to the length of the IR, so...

#+begin_src emacs-lisp
(let* ((bytesPerSample 2)
       (fs 48000)
       (irDuration .005)
       (irSamples (* irDuration fs)))
  (format "IR samples %f, %f byte/sec"
          irSamples
          (* bytesPerSample fs irSamples)))
#+end_src

#+RESULTS:
: IR samples 240.000000, 23040000.000000 byte/sec

Doesn't look too promising, frankly. Seems likely that I'll have to
explore more efficient, FFT-based convolution algorithms. Still, I'll
try the time-domain version first.

Entered on [2025-05-30 ven. 14:38]
**** Email exchange with Alexander Carôt      :WRITING:NETWORKEDAUDIO:PTP:
Having supplemented my SMC paper with a reference to one of the papers
suggested by [[*Reviewer 1][reviewer 1]] [cite:@carotGNSSbased2019], I contacted the
first author, one Alexander Carôt of NetJack fame
[cite:@carotNetjack2009]. It turns out that Prof Carôt (who kindly
congratulated me on my paper) was that very reviewer! I asked about
the lingering ppb drift that I'm seeing, and he added Christian Hoene
(third author) to the discussion. Here's the latter's contribution to
the discussion:

#+begin_quote
Yes, it was an idea from research. A GNSS synchronized sound card – it
would work especially well together with the golden cables and PCB.

As for the product, we decided not to implement it as it would account
to about one additional click in a typical session. Thus, it never
left the POC and research phase.

Good luck with your research
#+end_quote

I'm not sure what golden cables he's referring to, nor what this
additional click might be. Never mind, I suppose.

Incidentally, Dr. Hoene is associated with [[https://symonics.com/][Symonics]], a company with an
interest in networked audio. They have a product, [[https://symonics.com/fastmusic/][Fastmusic]], that's
based on [[https://www.soundjack.eu/][Soundjack]] (another Prof. Carôt system, apparently).

Entered on [2025-05-30 ven. 16:13]
** 2025-06 juin
*** 2025-06-01 dimanche
**** Ethernet Dongle Trouble                                :HARDWARE:PTP:
Another new Linux kernel version, another problem with the
dongle. Tried reinstalling the driver (for kernel 6.6.90), and the
module file is sat in
=/lib/modules/6.6.90-1-MANJARO/kernel/drivers/net/usb=, /and/ =lsmod=
suggests that =ax_usb_nic= is loaded, but =ethtool= isn't giving up the
goods

#+begin_src shell :results verbatim
ethtool -T enp85s0u2u3u1c2
#+end_src

#+RESULTS:
: Time stamping parameters for enp85s0u2u3u1c2:
: Capabilities:
: 	software-transmit
: 	software-receive
: 	software-system-clock
...
Another restart probably required.

Note also that the interface identifier has changed /yet again/. Good
thing I'm turning my back on the whole dongle situation.

***** Post-restart

First, let's list our ethernet interfaces
...
#+RESULTS:
: 6: enp85s0u2u3u1: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
:     inet 192.168.10.10/24 brd 192.168.10.255 scope global noprefixroute enp85s0u2u3u1
: 7: enp85s0u2u4: <NO-CARRIER,BROADCAST,MULTICAST,UP> mtu 1500 qdisc fq_codel state DOWN group default qlen 1000

OK, interesting, the old ID is back.
...
Entered on [2025-06-01 dim. 14:31]
**** Ananas: Initial Time Exchange                        :NETWORKEDAUDIO:
If I'm to dispense with the dongle, I need to set up some kind of
initial exchange of time between the clock authority Teensy and the
audio server. My first idea was just to add a thread to the audio
server to scrutinise the network for a PTP follow-up message, but that
poses a couple of problems:
- Messages are currently bouncing around on OSI level 2. Reading L2
  PTP packets on a raw socket requires an interface name, and I think
  opening a ~SOCK_RAW~ requires root permissions (possibly corroborated
  [[https://www.tenouk.com/Module43a.html][here]]).
- Socket programming is kind of horrible.
- Isn't Wireshark perfectly happy sniffing around for L2 packets?
  + Yes, but I'm pretty sure installing Wireshark adds a privileged
    group to the system so it's possible to access all the named
    network interfaces.

What about switching back to L3, now the dongle is out of the
equation? Anecdotally, this seems to make clock authority ambiguity
more likely.

Entered on [2025-06-01 dim. 18:04]
*** 2025-06-02 lundi
**** Ananas: Initial Time Exchange II                     :NETWORKEDAUDIO:
The plot thickens.

- So maybe if clock subscribers start before the switch, then we get
  into a whole ~Invalid PLL4 numerator 2147483647 for target sample
  rate 161842.881121 and denominator 1000000000~ situation. The
  solution there is restart PTP (maybe via the ~onControllerUpdated~
  callback).
- After bashing my head against a wall for a few hours (see "Socket
  programming is kind of horrible"), it became clear to me that JUCE's
  datagram socket implementation is not compatible with joining a
  multicast group via a *specific* network interface on a port that's
  bound to *any* network interface. Allow me to explain...

I asked [[https://phind.com][Phind]] about networking in JUCE, but it hallucinated at me for
a bit and I gave up. For some reason I thought of Claude, and give the
free model a try. After a false start, during which it insisted I
should be using port 319 (event port; incorrect) for follow-up
messages rather than 320 (general port; correct; it conceded its error
when I demonstarted a wireshark capture), it regurgitated the
following, more-or-less immaculate code:
...
public:
    ...
    bool setup(const std::string& multicastGroup, int port, const std::string& localInterface)
    {
        // Create socket
        socketFd = socket(AF_INET, SOCK_DGRAM, 0);
        if (socketFd < 0) {
            DBG("Failed to create socket: " << strerror(errno));
            return false;
        }
        ...
        if (setsockopt(socketFd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) < 0) {
            DBG("Failed to set SO_REUSEADDR: " << strerror(errno));
            close(socketFd);
            socketFd = -1;
            return false;
        }
        ...
        if (bind(socketFd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
            DBG("Failed to bind socket: " << strerror(errno));
            close(socketFd);
            socketFd = -1;
            return false;
        }
        ...
        DBG("Successfully joined multicast group " << multicastGroup << " on port " << port);
        return true;
    }
    
    int receiveData(uint8_t* buffer, int bufferSize, std::string& senderIP, int& senderPort)
    {
        struct sockaddr_in senderAddr;
        socklen_t senderAddrLen = sizeof(senderAddr);
        
        int bytesReceived = recvfrom(socketFd, buffer, bufferSize, 0, 
...
// Usage in your TimestampListener class:
void Server::TimestampListener::prepare()
{
    if (!multicastReceiver.setup("224.0.1.129", 320, "192.168.10.10")) {
        DBG("Failed to setup multicast receiver");
        return;
    }
...
void Server::TimestampListener::run()
{
    while (!threadShouldExit()) {
        if (multicastReceiver.waitForData(500)) {
            uint8_t buffer[1500];
            std::string senderIP;
            int senderPort;
            
            int bytesRead = multicastReceiver.receiveData(buffer, sizeof(buffer), senderIP, senderPort);
            
            if (bytesRead > 0) {
                DBG("Received " << bytesRead << " bytes from " << senderIP << ":" << senderPort);
                
                // Log first few bytes for debugging
                DBG("First bytes: " << std::hex 
                    << "0x" << (int)buffer[0] << " "
                    << "0x" << (int)buffer[1] << " " 
                    << "0x" << (int)buffer[2]);
...
The only bit that didn't work was ~// Log first few bytes for
debugging~. Blimey. Anyway, what was the problem with my original
JUCE-based attempt? The culprit: ~juce::SocketHelpers~:
...
Here, if an address (i.e. that of a network interface) is provided,
~addr.sin_addr.s_addr~ is set to that address, when in fact it needs to
be bound to ~INADDR_ANY~. So just omit the address right? Wrong, because
there's no way to provide an address to ~DatagramSocket::joinMulticast~
...
~lastBindAddress~? That's the value that gets passed to
~DatagramSocket::bindToPort~; it can't be overridden here.

So just call ~SocketHelpers::multicast~ directly, right? Wrong; all of
those helpers are defined statically in =juce_Socket.cpp=. The solution:
...
Entered on [2025-06-02 lun. 15:22]
**** Convolution Won't Work                                       :TEENSY:
To be specific, the PSRAM isn't fast enough to do time-domain
convolution. With an IR length of 256 samples (thus a convolution
buffer of two channels each of 256 samples), and appropriating the
~CYCLE_COUNTER_APPROX_PERCENT~ macro from ~AudioStream~, the algorithm
consumes 115% CPU time.

| IR length (samples) | Approx CPU time (%) |
|---------------------+---------------------|
|                   2 |                 1.6 |
|                   4 |                 2.5 |
|                   8 |                 4.3 |
...
So an approximate doubling with each doubling of the number of
samples. (That's linear, isn't it? I thought we were dealing with
\(O(N^{2})\)). Anyway, the irony is IRs of that length could be stored in
RAM and things wouldn't be nearly as slow. (I should check that
tomorrow.)

Entered on [2025-06-02 lun. 20:27]
*** 2025-06-03 mardi
**** Teensy RAM vs PSRAM                                          :TEENSY:
Alright, this is unexpected. Switching to RAM, 64-sample IR:
#+begin_example
Convolver: 37.580002 % CPU
#+end_example

and, to restate the point, using the PSRAM:
#+begin_example
Convolver: 29.420002 % CPU
#+end_example

What the bloody hell is going on?

First things first, it was a bad idea to implement a ring buffer with
direct pointer arithmetic, e.g.

#+begin_src c++
EXTMEM int16_t convBuffer[Convolver::kBufferSize][Convolver::kNumChannels]{};

int16_t *Convolver::FIFO::getReadPointer()
{
    const auto current{readPointer};
    --readPointer;
    if (readPointer < convBuffer[0]) {
        readPointer = convBuffer[kBufferSize - 1];
    }
    return current;
}
#+end_src

(and similar for ~getWritePointer~). Incrementing/decrementing pointers
that reside on the PSRAM is costly, apparently. Doing no processing, just
iterating, the convolver reports

#+begin_example
Convolver: 4.352000 % CPU
#+end_example

and using ~size_t~ indices instead:

#+begin_example
Convolver: 0.088000 % CPU
#+end_example

Entered on [2025-06-03 mar. 09:45]
**** Réunion Emeraude: Simon Jacquin, Stagiaire     :MEETING:PRESENTATION:
/Évaluation des stratégies d'ordonnancement (scheduling) dans le
compilateur FAUST et pistes d'amélioration/

- For a given Faust program there may not be a unique valid
  implementation. Do these different implementations differ in
  performance?
  + (This is about the results of compilation, not compilation
    itself.)
- Graph representation; something about restructuring the graph in
  acyclic form, splitting out/removing loops with delays greater
  than 1.
  + Looks like a graph of dependencies.
  + /Partial graph/, says Christine. /Precedence graph (graph de
    précedence)/. /Topological tree/.
- Four strategies described for traversing the graph; depth-first,
  breadth-first, interleaved, reverse breadth-first.
  + There is no one best strategy; different Faust algorithms are
    more-efficiently handled by the different strategies.
  + Interleaved is pretty bad almost across the board... but there are
    some exceptions.
  + These also vary wrt. compiler (clang vs gcc).
- All compilers reschedule instructions; all use the /List Scheduling/
  algorithm.
- Intention is to focus henceforth on investigating clang in "native
  mode".
- /La vectorisation ne gagne pas toujours./
- Next: separate loops for better cache-usage...

Entered on [2025-06-03 mar. 10:06]
*** 2025-06-04 mercredi
**** CCRMA VSR: no dice; AES AIMLA acceptance              :WRITING:VISIT:
The [[*France-Stanford Institute][France-Stanford Institute]] turned down my request for cash
#+begin_quote
I am sorry to report that the center received many more applications
worthy of funding than our resources enable us to support and that we
are therefore unable to provide you with funding at this time.
#+end_quote
thus perish my hopes of being a visiting student researcher at
CCRMA. Should hopefully go back for a week (or two) with Romain in
October/November.

My Faust Autodiff paper was accepted, however; reviews: 2, 2, 2, -1. A
couple of the reviewers failed to appreciate 

Entered on [2025-06-04 mer. 14:54]
*** 2025-06-05 jeudi
**** More convolution on the Teensy                               :TEENSY:
Ditching iteration over channels (i.e. assuming two output channels
rather than generalising), and bumping the PSRAM clock speed from 88
to 105.6 MHz (see =startup.c=) it's possible to convolve two channels
with an IR of 750 samples (~15.6 ms). That's still dreadful, but I've
yet to explore more efficient convolution algorithms, so all hope is
not yet lost.

Also that's for the case that entails performing all DSP on the first
of two per-buffer DMA interrupts. I don't know why ~AudioStream~ does it
this way, but that's the approach I emulated. Anyway, this means that
I can't exceed 50% CPU time (the above costs just over 48%). In
effect, it might be possible to double the above IR length. Still
useless, but what I'm getting at is there's some unused scope for
computation.

On a side-note, my convolver appears to be back-to-front, i.e. setting

#+begin_src c++
irBuffer[0] = 1.f;
irBuffer[kBufferSize - 1] = 1.f;
#+end_src

adds a copy delayed by one sample, not the length of the IR buffer,
whereas
...
Creates the expected delay. I've certainly done something idiotic in
my code to reach this unintended outcome.

Entered on [2025-06-05 jeu. 10:48]
  
  [[file:~/org/journal.org::*Teensy RAM vs PSRAM][Teensy RAM vs PSRAM]]
**** Important things to do and maybe not do                   :BRAINDUMP:
***** DONE Reply to Cristina Rottondi
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-06 ven. 09:41]
:END:
Mention our upcoming meeting, the prospective IS2 workshop, CCRMA not
working out, etc.
***** DONE Reply to Jens
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-06 ven. 09:06]
:END:
Regarding drift simulation. Maybe explain that I've got a lot on my
plate and that stuff might have to wait till August.
***** DONE Reply to Leonardo
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-06 ven. 09:06]
:END:
Thank him (and Jens) for their help with my CSI.
***** DONE Submit CSI report to ADUM
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-06 ven. 14:44]
:END:
And any other required supporting documentation, naturally.
***** DONE Contact Chris & Nette
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-06 ven. 09:58]
:END:
The VSR game is up.
***** DONE Prepare slides for SMC
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-01 mar. 17:29]
:END:
Just 5 minutes. Use Typst? In that case, ask Pierre for a template.
***** DONE Prepare poster for SMC
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-04 ven. 18:25]
:END:
And ask Tanguy about printing it to foldable fabric instead of paper.
***** DONE Register for AES AIMLA
:LOGBOOK:
- State "DONE"       from "WAITING"    [2025-07-01 mar. 17:29]
- State "WAITING"    from "TODO"       [2025-06-19 jeu. 17:37] \\
  Just applied for student membership, which will bring the registration
  fee down considerably.
:END:
Do this by July 1st (early-bird deadline). [[https://aes2.org/event-extra/2025-aes-international-conference-on-artificial-intelligence-and-machine-learning-for-audio/][$475 for non-members]].
***** DONE Mission for AES AIMLA
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-24 jeu. 11:09]
:END:
In Portail Missions, state that the source of funds should be /dotation
d'Inria/.
***** DONE Submit camera-ready AES paper
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-24 jeu. 11:19]
:END:
Do this by +July 14th+ [[https://aes2.org/contributions/2025-1st-aes-international-conference-on-artificial-intelligence-and-machine-learning-for-audio-call-for-contributions/][July 24th]].
***** DONE Finish 3rd year enrolment with doctoral school
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-06-17 mar. 11:56]
:END:
Needs a combined PDF containing
- Progress report
- Letter from Tanguy (verdict on progress, perspectives for following
  year)
- Convention individuelle de formation doctorale
  
Entered on [2025-06-05 jeu. 11:15]
*** 2025-06-08 dimanche
**** Even more convolution                                    :DSP:TEENSY:
I had indeed done something stupid when resetting the convolution
buffer read index; it's no longer an issue.

So I managed to port the Teensy Audio Library's 1024-point FFT to my
~AudioProcessor~ model, and it works, /and/ it's fast --- like 5% CPU fast
--- thanks to the ARM Core M7's SIMD capabilites. This is great, and
could support frequency-domain convolution (well, multiplication), but
equally I might well be able to wring more performance out of the
time-domain approach before I have to resort to something like
singular value decomposition [cite:@atkinsApproximate2013].

There are also a load of built-in ARM DSP [[https://arm-software.github.io/CMSIS_5/DSP/html/group__Conv.html][functions]].

Entered on [2025-06-08 dim. 19:29]
  
  [[file:~/org/journal.org::*More convolution on the Teensy][More convolution on the Teensy]]
*** 2025-06-11 mercredi
**** Convoluted                                               :DSP:TEENSY:
It emerges that the ~arm_conv_...~ functions aren't strictly suitable
for real-time processing (so says Claude🤖, so pinch of salt); the
expectation is probably more that one would use ~arm_fir_...~ instead;
there's an [[https://arm-software.github.io/CMSIS_5/DSP/html/arm_fir_example_f32_8c-example.html][example]], but that features a filter of only 29 taps --- not
the thousands of taps I'm hoping to be able to work with. I got it to
work with an IR of 256 samples (10% CPU time for two channels), which
suggests a 2-4x potential speedup compared with my plain, non-SIMD
implementation --- great, but still useless. In any case, I can't
actually get it to work for anything greater than that; the cause is
entirely mysterious, but my guess (given the hard, power-of-two limit)
is something to do with caching or memory alignment.

As Romain pointed out yesterday, convolution reverbs (I could do with
an example or two when it comes to writing about this one day) are
invariably [[https://www.analog.com/media/en/technical-documentation/dsp-book/dsp_book_ch18.pdf][FFT-based]].

***** Resources
- [[https://www.analog.com/media/en/technical-documentation/dsp-book/dsp_book_ch18.pdf][Chapter 18]] from /The Scientist and Engineer's Guide to Digital Signal Processing/.
- A signal processing Stack Exchange [[https://dsp.stackexchange.com/questions/74710/convolution-reverb-calculation][discussion]] featuring a
  pseudo-MATLAB FFT convolution implementation.

Entered on [2025-06-11 mer. 10:42]

*** 2025-06-12 jeudi
**** Artificial Intelligence and Creativity in Music :LECTURE:PRESENTATION:
A presentation by (friend of the Emeraude team) Jérémy Cohen of
CNRS/CREATIS/Lyon 1; part of the [[https://arqus-alliance.eu/news/2nd-edition-journey-through-artificial-intelligence-lecture-series/][A Journey through Artificial
Intelligence]] lecture series.

- (I see there are going to be SUNO examples. This might not be to my
  taste.)
- How AI/ML can be a benefit to creative practice; downsides of AI in this regard.
- Gen AI is SoTA for text & image (+video) problems; what about music?
- Ca. 2024 quality was good, respected stylistic tropes, but "not yet
  very realistic music"
- That was /academic research/, with published results; the likes of
  Suno, private companies who do not publish, facilitate the creation
  of commercial-sounding results, musical, etc.
- Suno aiming to give the "artist" more and more control.
- Gen AI
  + + Lowers technical barries to creating music
  + + Generates new ideas; collaboration between AI and artists
  + - Copyright infringement --- trained on commercial material
  + - Producers/artists replaced?
- Many tools besides gen AI; artists have already started to make
  music with AI.
- AI Song Contest; mixture of Gen AI (e.g. text-to-singing voice),
  pitch detection, timbre transfer, source separation,
  mixing/mastering, text to /texture/...
- Jérémy says tone transfer /allows you to play an instrument that you
  otherwise cannot/.
- (Oh, he's not a fan of the term /AI/; he'd rather say /machine learning/;
  that is a principled position.)
- Supervised learning: linear regression (approximate a curve with a
  straight line); learn a function from data samples. Training data \to
  model \to training.
  + Looks like he's giving a binary classification example... 
  + A neural nework (generic non-lineal model) can approximately
    reproduce the function that produces the aforementioned straight
    line (or some other, more complicated function in higher
    dimensions).
- Basic Pitch (Spotify research); onset detector, denoiser \to note
  detector \to MIDI file. Training data is pairs of MIDI data and
  corresponding audio. Not too expensive to train; a couple of hours
  on a couple of GPUs; quick at inference time too.
  + Still quite clunky.
- Yaboi Hanoi (ex-Magenta) won the AI song contest (2022), in part, by
  using tone/timbre transfer on a /Phi Nai/ (Thai traditional
  instrument) --- doesn't conform to western tuning --- mapped onto a
  sax.
  * Now the sax does something that a real sax can't.

Entered on [2025-06-12 jeu. 13:58]
*** 2025-06-13 vendredi
**** Convolution thoughts from the Teensy Forum               :DSP:TEENSY:
***** PSRAM limitations 
Paul Stoffregen [[https://forum.pjrc.com/index.php?threads/external-psram-usage-advice-on-the-teensy-4-1.76886/post-357846][says]]

#+begin_quote
The theoretical bandwidth is about 40 Mbyte/sec if using the default
88 MHz clock, or about 60 Mbyte/sec if you edit the startup code to
use 132 MHz.
#+end_quote

So I should look into achieving that higher clock speed (currently I
have ~105 MHz by changing the value of a divider register, IIRC. I
probably need to alter the clock source too). [Edit: it didn't appear
to work for me at 132 MHz.]

#+begin_quote
PSRAM also works pretty well for complex stuff that fits within the
Cortex-M7 32K cache. For example, if you're doing matrix multiply or
FIR filters or direct convolution reverb, expect to run into problems
if any 1 data set your program handles at a time grows larger than
32K.
#+end_quote

This might explain why ~arm_conv_fast_q15~ wouldn't go any further than
256 samples (unpartitioned). I'll need to be wary of this while
implementing things in the frequency domain.

#+begin_quote
PSRAM can be accessed by DMA, which doesn't use the cache. Total
bandwidth is an issue.
#+end_quote

So, that sidestep the cache limitation, with other tradeoffs
presumably. Looks like the [[https://github.com/PaulStoffregen/teensy41_psram_memtest/blob/master/teensy41_psram_memtest.ino][memtest sketch]] uses DMA.

***** More PSRAM stuff

Probably of no direct use to me (besides debugging?) but interesting
nonetheless: someone [[https://forum.pjrc.com/index.php?threads/section-type-conflict-and-other-memory-error.76357/][trying to run a nerual network]] on a Teensy,
storing weights in PSRAM, using DMA for some operations.

***** Other attempts to synchronise Teensies

Convolution-related too. [[https://forum.pjrc.com/index.php?threads/synchronizing-teensies-again.75740/][First up]], attempting to record [[https://en.wikipedia.org/wiki/Green%27s_function][Green's
functions]] in two rooms, with a Teensy master in one room, synced to 32
teensies in another, and what [[https://forum.pjrc.com/index.php?threads/synchronizing-teensies-again.75740/post-348929][sounds like]] a convolution partitioned
across the group of 32...

Second (related) we have [[https://forum.pjrc.com/index.php?threads/triggered-i2s.74407/][attempts]] to /trigger I^{2}S/ at the same time
on distributed devices. Uses a voltage change on a given pin to
trigger I^{2}S operations, with I^{2}S buffers written too directly,
i.e. without using DMA. So... clock sharing over a physical link! Sort
of; not strictly the clock, and only syncing on startup, it seems,
i.e. no drift correction. But that's a [[https://forum.pjrc.com/index.php?threads/triggered-i2s.74407/post-339450][decent-looking sync]]. Damn.

What does that get me thinking? Well, PTP necessitates the use of
compliant switches. So if we had a clock authority producing a voltage
impulse train on a given pin, with that pin hard-linked to a
corresponding pin on each of the rest of the network (and there would
still have to be a /network/ for audio transmission), one could trigger
an interrupt that one could use, in tandem with the ENET clock /anyway/,
to do switchless PTP. From the thread (and because it's interesting):

#+begin_src c++
attachInterruptVector(IRQ_SAI1, isr);
NVIC_ENABLE_IRQ(IRQ_SAI1);

attachInterrupt(digitalPinToInterrupt(0), adjustClock, CHANGE);
#+end_src

Nonetheless, that extra cable... though it'd be daisy-chainable,
unlike ethernet... which would negate the elegance of the
daisy-chaining. Agh.

***** Partitioned convolution

Well, of course [[https://forum.pjrc.com/index.php?threads/low-latency-two-level-partitioned-convolution-for-the-t4.68939/][someone already tried it]].

#+begin_quote
I see about 28 to 35% ProcessorUsage on a T4.0 at 600MHz,
which is very usable. (with a single-tier partitioned convolution I
could only get to 20k taps or so before the CPU usage started to
max-out)

This 35% is at 44100 sample rate at 66161 taps, mono. If this could be
done in the time domain it would only allow 0.34ns per
multiply-accumulate operation.
#+end_quote

So... 70+% for ~1.5s stereo @ 48 kHz? Honestly, I don't know whether
to be optimistic. It's uniformly partitioned I guess... might be more
to be wrung out of it.

Entered on [2025-06-13 ven. 18:58]

*** 2025-06-17 mardi
**** Do we even need PSRAM?                                       :TEENSY:
Just a thought. A 5 s IR would fit in regular RAM.

#+begin_src emacs-lisp :results value
(let* ((irLengthS 5)
       (bytesPerSample 2)
       (fs 48000)
       (totalSamples (* irLengthS fs))
       (totalBytes (* totalSamples bytesPerSample)))
  (format "%d s, %d samples, %d B, %.1f kB, %.3f MB"
          irLengthS
          totalSamples
          totalBytes
...
#+RESULTS:
: 5 s, 240000 samples, 480000 B, 468.8 kB, 0.458 MB

Well, just, and that's if one assumes that there's not a lot of other
stuff needed in memory; not a given where, say, WFS is concerned.

OK, if there's a distinct IR for each of the two output channels then
that pushes things more in the direction of the PSRAM. And after
zero-padding and taking an FFT the above figures would likely grow.

Entered on [2025-06-17 mar. 14:10]
*** 2025-06-18 mercredi
**** What am I up to?                                          :BRAINDUMP:
I've taken a break from [[denote:20250530T165005][convolution]] for the time being to divert my
attention to:
- My [[file:presentations/smc-2025/presentation.org][slides]] for SMC
  + I decided to try to export from Org Mode to Typst for those. It's
    not a perfectly smooth experience so far, but it's fine.
- My poster for SMC
  + I haven't started this yet, but I found my 2023 SMC poster on
    overleaf.
  + There's also this niceish [[https://github.com/philipphoman/org-mode-poster][org-mode-poster]] example on github.
- The IS^{2} proposal
  + Cristina gave me some advice; said it shouldn't be too much
    work. N'importe quoi.
  
Entered on [2025-06-18 mer. 15:42]
*** 2025-06-20 vendredi
**** Cristina Rottondi, Torino visit                             :MEETING:
- Ask Tanguy/Romain about formal process.
  + And maybe Anouchka re. funding?
- Check civil liability insurance for validity abroad.
- Get letter from Cristina for formal purposes (at some point).
- Create account on /ospitalita/ [[https://ospitalita.edisu-piemonte.it/Login.aspx][website]].
- Peruse the halls of residence [[https://www.edisu.piemonte.it/en/services/living/halls-residences/hall-residence-borsellino][website]].
- No huge pressure to settle on the dates, but April 2026 suggested
  for now.
- Don't worry too much about MUSMET contribution; if something of
  mutual benefit emerges, great.
- Can be sorted out in the autumn!
- Opportunities to present my work, both in Turin, and perhaps also in
  Lugano and Milan.
- Sidenote, think about postdoc funding; funding opportunities in
  Switzerland, apparently (worth bearing in mind).

Entered on [2025-06-20 ven. 14:59]
*** 2025-06-26 jeudi
**** Linux Audio Conference: Nando & Nils    :PRESENTATION:NETWORKEDAUDIO:
/AVB For (Linux) Dummies/
...
I should be referring to /reproduction time/ as /presentation time/.

How does AVB do media clock correction? It doesn't? In that case how
is it achieved? PLL-conditioning or resampling?

Entered on [2025-06-26 jeu. 15:30]
** 2025-07 juillet
*** 2025-07-01 mardi
**** What's the latest?                                        :BRAINDUMP:
JIMLAC wiped out most of last week, productivity-wise, though I made
moves on my SMC slides, which I finished yesterday. These may well
need to be cut down to fit into five minutes.

The poster needs quite a lot of work, though I should be able to
re-use assets from the slides.

Unofficially (i.e. d'après Cristina R) the IS^{2} tutorial deadline, much
like the paper deadline, has been pushed back by two
weeks. Nontheless, unless I /finish/ that today, I should send an email
as per the [[https://internetofsounds2025.ieee-is2.org/call-papers/call-tutorials][instructions]] indicating that I have something in the works.

Entered on [2025-07-01 mar. 09:30]
*** 2025-07-02 mercredi
**** Romain                                          :MEETING:SUPERVISION:
***** DONE Order Teensies
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-04 ven. 18:25]
:END:
Nine of them, plus audio and ethernet shields. Find a good price, from
an EU distributor, preferably.
***** DONE Email Pierre Lecomte
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-04 ven. 18:25]
:END:
Can I come and test my system(s) et Cetrale in September?
***** DONE Arrange Stanford mission/transport/accommodation
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-08 ven. 16:30]
:END:
Choose dates that avoid Thanksgiving. Do [[*Mission for AES AIMLA][London/AES]] too. [Edit, don't
worry about thanksgiving; leave time for IS2, in case my tutorial is
accepted.]
  
Entered on [2025-07-02 mer. 16:41]
*** 2025-07-06 dimanche
**** Graz thoughts                                             :BRAINDUMP:
I have the awkward situation on my hands whereby IS^{2} is at the /end/ of
October, rather than the beginning (like last year), which now
corresponds with the period I'm due to spend at CCRMA, which should
ideally avoid Thanksgiving week (24-28 November). Options:

- Withdraw the tutorial proposal; go to CCRMA e.g. October 23th -
  November 22nd.
- Leave it be, go to CCRMA e.g. Nov 2nd-24th (~three weeks, as Romain
  suggested); hope that my proposal is accepted.

Do I even want to do the tutorial? It's work that isn't directly
related to my thesis, so obviously a drain on my resources, and yet
it'd be good for my /brand/, as it were.

I also have work to do, on my SMC slides and on the AES paper, which
may need (hopefully minor) revisions --- at the very least I need to
switch to the non-anonymised template.

***** DONE Decide CCRMA Dates; arrange mission
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-08 ven. 16:30]
:END:
See previous [[*Arrange Stanford mission/transport][TODO]].
***** DONE Edit down SMC slides
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-07 lun. 11:01]
:END:
And upload them to the appropriate shared drive (see email for details).
***** DONE Revise AES AIMLA paper
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-07-09 mer. 14:37]
:END:
Also in a previous [[*Submit camera-ready AES paper][TODO]].

Entered on [2025-07-06 dim. 08:12]
**** Bela Gem                                                   :HARDWARE:
So, there's a [[https://www.crowdsupply.com/bela/bela-gem-stereo-and-multi][new Bela]] in town. Quad-core, 1.4 GHz affair, 512 MB
LPDDR4, but the two-channel version is $97 with the associated
PocketBeagle. Cute that they assert, blithely, that the Teensy can
only run at 44.1 kHz.

Probably more than powerful enough for auralisation, but the price
puts it way out of reach as far as I can see.

Entered on [2025-07-06 dim. 16:16]
*** 2025-07-11 vendredi
**** Optimisation Residual Synthesis                               :INSPO:
I should try to generate some sonic material with faust-ddsp and
compose something. There's the unoptimisable phasor, the oscillator
example, and just getting the bursts of noise (or the residuals of
other input signals) from examples that optimise well could work well
too. I wonder what it would sound like to try signals besides
~no.noise~, e.g. musical excerpts. I could submit something to next
year's SMC. Theme of /loss/.

The procedure might be to run a local version of the web IDE, connect
it to Reaper with Pipewire, and record some samples.

Entered on [2025-07-11 ven. 09:04]
**** SMC Keynote: Jana Winderen                  :PRESENTATION:CONFERENCE:
[[https://smc25.iem.at/sessions/keynote11/]["Listening with"]]

Now, there's an idea: taking underwater ambisoics recordings to be
reproduced on dry land. Wonder whether I could get hold of some of the
recordings to test with.

P.S. formidable individual! Amazing drawings of the beasts (insects,
fish...) she's studied. Tons of field recordings, beautiful
photographs. Bone conduction example; great big tuning fork passed
around the room to be struck and brought to the top of one's head;
idea being to get us thinking about other species hear, and generally
sense the world around them. And how they make noise! Fish making
metallic, granular-like sounds (as picked up by a hydrophone).

Ecological impact on aquatic animals; they can be /deafened/ by
underwater industrial activity.

Material on [[https://janawinderen.bandcamp.com/][bandcamp]].

Entered on [2025-07-11 ven. 09:27]
**** New OTTOSonics paper                               :INSPO:CONFERENCE:
/LOTTO (Localize OTTOsonics): Automated Spatial Speaker Localization
Using Tetrahedral Microphone Array/ ([[https://smc25.iem.at/contributions/lotto/][link]])

So, they've solved DIY calibration, using Cramer's rule... practically
instantaneous; exportable to the IEM plugins. It's in Pd, however, and
in centralised form. I wonder whether I can adapt it to the
distributed case.

Entered on [2025-07-11 ven. 10:50]
*** 2025-07-25 vendredi
**** PPB Drift Fixed                           :NETWORKEDAUDIO:TEENSY:PTP:
Method:
- Count audio interrupts.
- After a second's worth of the above (2 * 375 for a buffer size
  of 128), read the ENET timer for the nanoseconds part.
- Compare with the nanoseconds registered the first time a second's
  worth of interrupts occured.
- In a perfect world the difference between ~currentNS~ and
  ~firstInterruptNS~ should be zero; in practice it is not. In any case
  store the difference.
- When adjusting the audio clock, include ~audioPTPOffset = currentNS -
  firstInterruptNS~ in the calculation.
  #+begin_src c++
const double proportionalAdjustment{
    1. + (adjust + s_AudioPTPOffset) * ClockConstants::k_Nanosecond
};
  #+end_src
  where ~adjust~ is the PTP-derived nsps value.

And this works great, for the most part. I've had three Teensies
running most of the afternoon, mostly staying synced to within 250 ns.

Occasionally, however, a given device can get into a sort of
oscillation, where the audio/PTP offset correction becomes rather
wild. Amplitude can be as much as 500 ns. Not the end of the world,
but it would be nice to avoid that. I briefly tried implementing a PI
controller by way of mitigation, but it didn't work. I should try
again.
  
Entered on [2025-07-25 ven. 18:30]
** 2025-09 septembre
*** 2025-09-08 lundi
**** AES AIMLA: Franco Caspe, Jatin Chowdhury   :PRESENTATION:CONFERENCE:
[[https://aes2.org/event-extra/2025-aes-international-conference-on-artificial-intelligence-and-machine-learning-for-audio-program/][Tutorial]]: /Real-Time Neural Audio Inference/

Challenges of real-time inference, etc. Requirement to store state for
block-based processing, and to match kernel size (i.e. length of CNN,
which is analogous to FIR filter) to buffer size, or alternatively
zero-pad so things don't get uncausal --- standard convolution
requirements.

(Incidentally, an IIR filter is analogous to an RNN, and I should
really have twigged, having read, /and cited/, B. Kuznetzov's paper on
differentiable IIR filters.)

Anyway, interesting Colab notebook [[https://colab.research.google.com/drive/1GBVoNxyBoMIKsuy6f9B8uLF2WYgwKrhm][here]]. Example of CNN with several
convolutional layers, each one with a /dilation/ applied; the first is
essentially a 3-tap FIR filter (followed by a leaky ReLU), the second
has three non-zero taps, each separated by a zero-tap, for the nth,
each of three non-zero taps is separated by \(2^{(n-1)}\) zero taps. Used
for modelling a guitar fuzz effect.

Entered on [2025-09-08 lun. 09:57]
*** 2025-09-24 mercredi
**** WFS control plugin                                   :NETWORKEDAUDIO:
/On y go/. So, initial thoughts.
- In a DAW context it isn't possible to query the active audio output
  device, so it's not possible to know by that means whether /Teensy
  Ananas/ is connected.
- Without configuring the switch to mirror traffic...
  #+begin_example
> interface/ethernet/switch/port set ether[N] mirror-ingress=yes
  #+end_example
  ...it's not possible to read PTP messages transmitted by Teensies on
  the network, so that's not a way of knowing whether /Teensy Ananas/ is
  connected, nor whether other Teensies are.
- I guess sending a beacon packet (to =192.168.10.10= or whatever
  address has been assigned to the ethernet interface, or to a
  multicast group, or the same multicast group on a different port...)
  once per second, say, would do the job of announcing the presence of
  /Teensy Ananas/, but obviously wouldn't say whether it's the active
  audio device.

What happens if a different device is in use?
- The plugin takes initial timestamp from a follow-up packet and uses
  that to compute the start time for the outgoing packet stream.
- Successive audio callbacks at a rate governed by the active audio
  device, not by /Teensy Ananas/.
- Inevitable drift occurs between the packet stream and the audio
  stream on the Teensy network.

Another thing to consider is Pipewire, specifically how =pw-jack= is
configured. Configuration is handled by =~/.config/pipewire/jack.conf=,
and I'm most interested in this bit:
...
Those should really be =128=... or rather it shouldn't matter, Ananas
Server should adapt its packet structure to the quantum and the client
should decouple the packet stream from the audio stream.

Entered on [2025-09-24 mer. 14:43]
*** 2025-09-25 jeudi
**** Presentation time problems                :NETWORKEDAUDIO:TEENSY:PTP:
In a DAW context we have the issue that /pressing play/ (or /unfocusing
the DAW/) can result (at least in the case of Reaper) in packet
timestamps exceeding the available range at the client side. This, I
suppose, is because of some sort of buffering delay at the server side
that means the expected increment (2,666,666.6... ns at 128/48000)
doesn't match the amount it takes to process the first (few)
buffer(s). The delay tends to be on the order of 0.1 s.

A solution of sorts is keep the server's timestamp listener thread
running and compare, every time a follow-up packet is received, the
time of the follow-up packet with that of the current outgoing packet
timestamp. Given that the client-side packet buffer is 50 packets
long, if the absolute value of the time difference is greater than 25
times the expected packet interval, use the new timestamp (with the
usual presentation offset of .1 s applied).

This sounds like it ought to be kind of dreadful, but actually as long
as the DAW is playing or stays either in or out of focus, things stay
pretty stable: the server doesn't get out of sync, and clients stay at
a steady packet/sample offset.

An alternative approach would be to have clients reporting their
packet/sample offset over the network. This could be part of the
information that they send as part of the periodic "I am here!"
message that I intend to implement. Should a given client report that
it's not able to find a packet with a timestamp that's valid with
respect to its PTP timer, it could announce whether it's behind or
ahead and the server could act accordingly (re-open the timestamp
listener thread?). If one client gets out of sync, it's fair to assume
that the whole group are suffering, so this should be a viable
approach.

Anyway, let's see how we get on with the current approach.

Entered on [2025-09-25 jeu. 18:35]
** 2025-10 octobre
*** 2025-10-07 mardi
**** More on presentation time                            :NETWORKEDAUDIO:
The situation I was encountering was one whereby [[*Presentation time problems][presentation time
problems]] were occurring in the DAW, but not for the console app. And
that was with the DAW in the /foreground/, on the order of once a
minute; quite troubling. What's the difference? Aren't they both using
JACK? Well, perhaps, but the DAW is obviously a lot heavier. I decided
to try to experiment with pipewire's JACK config
(=~/.config/pipewire/jack.conf=). I had both =node.latency= and
=node.quantum= set to 128; doubling =node.latency= seems to have a
positive impact:
...
No timing issues (besides on initial playback) during the past ten
minutes or so. Promising. One to keep an eye on though.

Entered on [2025-10-07 mar. 13:19]
**** Presentation time again                              :NETWORKEDAUDIO:
So, even that was maybe a bit ambitious; setting the quantum to 256
too appears to help.
...
Since I've decoupled the audio buffer in the server from the packet
size, it just works, with the caveats that it's necessary to drop the
reproduction offset to one sixty-fourth of a second

#+begin_src c++
constexpr static int64_t PacketOffsetNs{NSPS / 64};
#+end_src

and the packet sleep interval to an eighth of the expected packet
interval.

#+begin_src c++
nsSleepInterval = nsPerPacket * 1 / 8;
#+end_src

Too long a sleep and the send thread doesn't get all the packets out
in time and the clients get into difficulty.

***** Addendum
Actually even 1/8 seems too much; maybe a delay isn't necessary? I'm
trying 1/64 at the moment. Seems stabler. Seems to work without the
delay too...

Entered on [2025-10-07 mar. 16:44]

*** 2025-10-13 lundi
**** More presentation time + USB woes                    :NETWORKEDAUDIO:
Alright, I asked my dear friend Claude about optimising my system such
that Reaper might behave itself a little better; towards the bottom of
[[https://claude.ai/share/68d6ed4e-d31c-435e-8982-03109213cf96][this discussion]] came the following advice:

/Set CPU to performance mode to prevent frequency scaling stutters:/
...
#+begin_src shell :noeval
# Make permanent - install and enable
sudo pacman -S cpupower
sudo systemctl enable --now cpupower.service
#+end_src

Running as a service, cpupower is configured via =/etc/default/cpupower=.

The above, changing the setting from =powersave= to =performance=, really
seems to do the job; for now I'll stick to doing it manually, but at
some point I should incorporate this advice into the (as-yet
nonexistent) documentation for the system.

***** USB audio trouble
Clients are staying nicely synchronised for the most part, but their
packet buffers seem to be inexorably filling up to the point of
overflowing. This suggests that TeensyAnanas (the USB audio device,
should any reminder be required) is asking my laptop (more
specifically Pipewire I suppose) for audio /too often/, i.e. the average
audio interrupt interval on my laptop is shorter than it should
be. I've tried tinkering with
=framework-arduinoteensy/cores/teensy4/usb_audio.cpp= but so far to no
avail.

Entered on [2025-10-13 lun. 14:09]

*** 2025-10-16 jeudi
**** Soutenance de thèse: Romain Pichon (MARACAS)             :SOUTENANCE:
*Quantum Algorithms for NOMA Systems*

/Design and evaluation of quantum annealing-based solutions for
massive access in wireless networks/

#+begin_note
Romain put bottles of water out on the desks for each member of the jury...
#+end_note

- Massive machine type communication systems; 10^{4} - 10^{6} devices served simultaneously.
- The idea is try to leverage quantum computing to better serve such systems.
- Strong connection between physics and decoding tasks; investigation
  of use of quantum algorithms for signal processing.
- 5G wireless networks; multiple users attempt to initiate uplink
  simultaneously.
- Communication channel has a /frequency band/ and /frame duration/.
- Access techniques; access protocols.
- /Code division multiple access/ --- multiple users can share resource
  blocks.
- /Active User Detection/ (AUD) not a trivial task. Non-orthogonality of
  activity signals. Most reliable estimator of activity:
  /Maximum-Likelihood/ (minimises a log-likelihood). Gradient descent
  involved...
- Maybe qubits are better suited to this sort of problem...

I understood very little of the technical detail, but by all accounts
an incredibly assured and professional job; very clearly presented.

Entered on [2025-10-16 jeu. 09:03]
*** 2025-10-17 vendredi
**** More USB audio trouble                        :NETWORKEDAUDIO:TEENSY:
I had to take a step away because it was driving me loopy. Issue is it
doesn't seem to matter what I do with the ~feedback_accumulator~ of
~AudioInputUSB~, my computer seems to process audio at a rate faster
than one would expect, with PTP as the reference for authoritative
time, i.e. client buffers overflow, which suggests that the computer
is sending audio too quickly, which in turn suggests that the
time-authority Teensy is requesting audio too quickly.

Claude gave me [[https://claude.ai/share/8230aaf2-e122-45c2-8798-4fb9d02db5cf][some leads to follow]], e.g. checking that the computer
is receiving USB frames containing the feedback accumulator. It also
led me to check whether Teensy runs as a high-speed USB device or not;
I was under the impression that it doesn't, but (having added a
~usbHighSpeed~ member to ~AuthorityAnnouncePacket~) it turns out that it
does. This in turn suggests that certain elements of Teensy's USB
descriptor (=usb_desc.c=) are incorrect, i.e.

#+begin_src c
0x11,  // bmAttributes = isochronous, feedback
3, 0,  // wMaxPacketSize, 3 bytes  <-- WRONG for High Speed
1,	   // bInterval, 1 = every frame
5,	   // bRefresh, 5 = 32ms

// What It Should Be for High Speed
0x11,  // bmAttributes = isochronous, feedback
4, 0,  // wMaxPacketSize, 4 bytes
4,     // bInterval, 4 = every 8 microframes (1ms)
0,     // bRefresh is ignored for High Speed
#+end_src

At one point I had the suspicion that audio and PTP mighte be drifting
relative to each other on the timing authority. Using an
~audioPTPOffset~ field in the packet struct, however, I discovered that
this is not the case; the greatest offset I saw during a couple of
hours of testing was \pm40 ns (which is one tick of the 25 MHz ethernet
timer, and totally conceivable if there's a slight offset between
reading the timer and comparing with the time at the point at which
~2 * samplingRate / bufferSize~ (750 @ 48 kHz @ 128 samples) frames have
elapsed). So, we come back to the USB feedback accumulator, and
whether the computer is respecting it.

To offer a more positive reflection on recent work, I now have the
foundations for monitoring a network of clients from within a
standalone app or a plugin, I can verify (more-or-less accurately)
that they reliably achieve a PTP lock, and regain it if it is
disturbed; I've even written the client implemtation such that it
recovers from a catastrophic loss of authoritative PTP time (in which
case it restarts the PTP algorithm and retries). A group of five
clients stays synchronised to with a couple of hundred nanoseconds
during "normal use", i.e. with the audio server running in a CLAP
plugin. CPU use on the client is less than 1 % for packets containing
16 channels. In principle, with 32-frame packets we could send up to
22 channels, or 44 @ 16-frames, but at 16 timing becomse more fraught
(the packet buffer is half the /duration/, and the risk of
overflow/underrun is greater; increasing the /size/ of the buffer puts
pressure on Teensy's limited memory... though maybe using an attribute
~PSRAM~, ~DMAMEM~, etc., I could get it to reside somewhere more
sensible), and handling more channels, more frequently, eats more CPU
time.

Entered on [2025-10-17 ven. 19:23]
** 2025-12 décembre
*** 2025-12-04 jeudi
**** Ananas Latest                                        :NETWORKEDAUDIO:
It's been a while; I haven't been well.

I got Teensy Ananas (the PTP/USB audio time authority) working better
in Reaper. Much tinkering with teensy core =usb_audio.cpp= led me to
come up with my own scheme for handling USB audio feedback (see
~FEEDBACK_ACCUMULATOR~) and the following useful terminal commands:
...
#+RESULTS:
#+begin_example
Teensyduino Teensy Ananas at usb-0000:55:00.0-2.3.3.2, high speed : USB Audio
...
which can be used with =watch=, i.e.

#+begin_src shell :noeval
watch -n 0.1 'cat /proc/asound/card2/stream0'
#+end_src

In any case, this demonstrates that feedback is in the correct format,
the momentary frequency is slightly evelvated in this case, etc.

Another useful command:

#+begin_src shell :noeval
sudo cat /sys/kernel/debug/usb/usbmon/5u | grep -E "Zo.+[0-9]{2}[^2] = "
#+end_src

Which generates output like this:

#+begin_example
ffff8881e75ea100 2641147579 S Zo:5:025:3 -115:8:3816 1 -18:0:196 196 = 4b00e3ff 4c00dcff 4e00daff 4e00d2ff 4900b7ff 440097ff 410068ff 410043ff
ffff8881e75ea100 2643195598 S Zo:5:025:3 -115:8:3816 1 -18:0:196 196 = 3d008e00 4a009900 4d00a200 4400a000 3e00a000 3d00a000 3c00a000 3900a300
ffff8881e75ea100 2645243634 S Zo:5:025:3 -115:8:3816 1 -18:0:196 196 = 61010201 0c027201 18026201 a4010201 4901c100 4401d500 5e010d01 64013f01
ffff8881e75ea100 2647291650 S Zo:5:025:3 -115:8:3816 1 -18:0:196 196 = d3ff1e00 ceff1400 c9ff0a00 c1ffffff b3fff8ff a4fff3ff 9dfff2ff 94fff5ff
ffff8881517f5000 2651373682 S Zo:5:025:3 -115:8:3704 1 -18:0:196 196 = a7fd04fe a9fd0afe aafd0bfe a7fd11fe a4fd16fe a8fd19fe a7fd1afe a7fd17fe
ffff8881517f5c00 2651407678 S Zo:5:025:3 -115:8:3976 1 -18:0:188 188 = d9017401 d7017401 d9017801 d8017e01 da018301 dd018801 dc018b01 d9019201
ffff8881517f5c00 2659599757 S Zo:5:025:3 -115:8:3976 1 -18:0:188 188 = a7ff55ff 71ff6aff 31ff73ff f3fe7aff cafe6fff b3fe5aff a4fe3eff a3fe20ff
ffff8881517f5c00 2667791837 S Zo:5:025:3 -115:8:3976 1 -18:0:188 188 = 7f00f5ff 8000f5ff 8300f6ff 8300fcff 8300faff 8800ffff 8500fdff 8a000100
ffff8881517f5c00 2675983918 S Zo:5:025:3 -115:8:3976 1 -18:0:188 188 = 46fcd1fc 3ffcd1fc 39fccdfc 34fccefc 30fcc9fc 2cfccbfc 2afccafc 27fcc8fc
ffff8881517f5c00 2684176008 S Zo:5:025:3 -115:8:3976 1 -18:0:188 188 = 0100edfe 1700f9fe 2c00fcfe 3d0004ff 53000dff 650019ff 82002bff a00030ff
#+end_example

The expected USB packet size at 48 kHz is 192 bytes. Filtering out the
packets of that size, we see that, as the momentary frequency changes,
the device occasionally requests packets that are either four bytes
larger (running fast) or smaller (running slow). At first this wasn't
working because I'd set a hard limit on packet size, in =usb.c= of 192,
not understanding that some headroom is needed for when the feedback
accumulator is greater than its default value of ~48 << 25~. Anyway,
that all seems to work pretty nicely now, and USB audio more-or-less
keeps pace with the PTP-based recovered media clock, i.e. client
buffers don't overflow.

**** Using ALSA instead of JACK

The whole USB audio feedback mechanism seems to track more reliably to
PTP time when using Teensy Ananas as Reaper's audio device via ALSA
directly, rather than using JACK (strictly =pw-jack= in my case). It's
also necessary to specify zero input channels for things to behave nicely.

**** Preventing spurious timestamp updates

Since the JUCE app/plugin is listening for PTP follow-up messages
(which are timestamped) on an ethernet interface without
hardware-level PTP support, there are non-deterministic delays in the
time at which such packets are received and processed by the
application. These delays can vary from anything as low as a few
hundred microseconds to a few miliseconds, particularly as USB Audio
drifts relative to PTP. This is acceptable up to a point; at 32-frames
and a packet buffer size of 50, clients have \pm16.6 ms or thereabouts
grace to play with. That being said, occasionally what I've observed
is the server finding PTP timestamps that are far outside of that
range. This obviously happens when hitting play in Reaper, but it can
also happen for no readly apparent reason during playback. I observed
that it tends to be a case of one timestamp that's wildly late,
followed by one that appears to be wildly early, which is to say that
by the time the third timestamp is read, we're back where we
started. So, the solution, for now, is that only if three /bad
timestamps/ in a row are encountered should the outgoing packet
timestamp be updated. This means a delay of 3-4 seconds between
hitting play in Reaper, and clients being ready to reproduce audio
(they have to respond to any such change in packet timestamp at their
end), but that simply entails leaving some silence at the beginning
of a Reaper session.

**** Loss of PTP lock

I need to investigate this further, but it seems that if a given
client loses its PTP lock --- i.e. it slips beyond \pm 100 ns --- then
it can lose its presentation time parity with the rest of the
network. The only way I can think to test this is to unplug the
ethernet cable for a given client and see how it responds. I don't
know yet how to correct for this if it occurs. In principle I don't
think it should be happening at all that presentation time doesn't
correct itself once a PTP lock is restored, but there it is. The vast
majority of the time I witness no such problem, by the way; i.e. the
four clients I have connected to the logic analyser can happily remain
synced to within a couple of hundred nanoseconds for hours at a time.

**** Multicast address weirdness

Having consulted with Claude, I decided to move authority and client
announcement packets away from the multicast address I'm using for
audio, i.e. =224.4.224.4=, with the rationale that I don't necessarily
want to be placing any additional burden on devices joining that
multicast group --- clients don't need to receving authority
annoucement packets for example, nor announcement packets from other
clients. However, trying =224.4.224.6=, for example, I find that,
although these packets show up in Wireshark, my plugin doesn't receive
them. It's really bothering me. I tried other addresses too
(=224.4.226.6=, =224.44.224.44=, etc.) but none of those work either. Did
I configure the switch in some way when we first got it? No... if it
was that then Wireshark wouldn't see those packets. There's something
weird going on with my C++ (or with JUCE) I think.

Entered on [2025-12-04 jeu. 15:52]

*** 2025-12-05 vendredi
**** Multicast weirdress fixed
Why was the PTP follow-up listener thread still working? It wasn't an
address or a port or an interface problem, it was that bug with JUCE
whereby if one wants to bind to ~INADDR_ANY~ /on a specific interface/
it's necessary to work with the raw socket and ~setsockopt~ directly.

**** Recovering from loss of PTP Lock
The nuclear option is to reboot the Teensy
...
then set it up again completely when the clock restarts. For reasons
that escape me, it doesn't work if I wait until PTP is ready to
initialise DMA in the first place; it's necessary to do it when
setting up ~AudioSystemManager~.

Entered on [2025-12-05 ven. 16:57]

* 2026
** 2026-01 janvier
*** 2026-01-10 samedi
**** Two switches                                     :NETWORKEDAUDIO:PTP:
Fired up a second [[file:~/org/journal.org::*MikroTik Cloud Router Switch][MikroTik switch]]. These can act as a [[https://help.mikrotik.com/docs/spaces/ROS/pages/64127015/Precision+Time+Protocol#PrecisionTimeProtocol-Summary][Two-step
Ordinary Clock and Boundary Clock]], and not, as described in
[cite:@vankempenIEEE], as a transparent clock.
#+begin_quote
Due to the cumulative effect of multiple servo loops, cascading
multiple Boundary Clocks within a network can significantly degrade
clock accuracy of the system. In order to avoid this, a new clock type
called “Transparent Clocks” (TC) appears in PTP V2.
#+end_quote
Consequently (as far as I can tell) I'm seeing a 400-500 ns offset
between a Teensy connected to one switch versus one connected to the
other. I have confirmed that this is a PTP issue (and not one of audio
transmission, which wouldn't make sense anyway) by connecting the
logic analyser to pin 24 (~ENET_1588_EVENT1_OUT~) on Teensies connected
to the two switches; sure enough, ~400 ns offset. I tried swapping out
the daisy-chaining ethernet cable for a much longer one to see whether
that might be a factor, but apparently not. I don't think there's a
sensible workaround. Perhaps playing with that daft /hardware offset/
value in =ptp-base.cpp=...
#+begin_src c++
const int hwOffset = -200; // Hardware Offset
#+end_src
...but even if that were to work, it would require compiling different
versions of the clock subscriber on a per-switch basis, and connecting
them to the right switches. Hardly worth it over a matter of half a
microsecond. I wonder how things would look with a third switch in the
setup. For now I'm not even going to try it because I have other
things to do. Two switches gives us this many potential output
channels to play with:
#+begin_src emacs-lisp
(let ((numSwitches 2)
      (portsPerSwitch 24)
      (outputChannelsPerPort 2)
      (switch1PcPorts 1)
      (switch1DaisyChainPorts 1)
      (switch2DaisyChainPorts 1))
  (* outputChannelsPerPort
     (- (* portsPerSwitch numSwitches)
        switch1PcPorts
        switch1DaisyChainPorts
        switch2DaisyChainPorts)))
#+end_src
...
***** In other news
I fixed (sort of) switch-monitoring in the /Network/ tab of the
plugin. Those text fields still really don't like to receive focus,
which is a nuisance (but one I recall afflicting Amati++ in 2024);
limitation of JUCE, or JUCE+CLAP, and one that's a misery to debug, so
for now I'm leaving it be. Moving away from ascending numerical
identifiers for the switches, and just generating a random integer
when adding a row to the table instead, appears to have done the
trick. So now that tab consists of
- A /Switches/ section, where rows can be added, IP/username/password
  entered, drift/offset viewed, and a PTP reset command (actually a
  disable/re-enable) sent. Rows can of course be removed from the
  table.
- A /Time Authority/ section for keeping an eye on the Teensy that's
  acting as the PTP authority and USB audio device. This section only
  really exists because I was concerned about the whole /feedback
  accumulator/ situation and wanted to keep tabs on it; it's probably
  not of great use in the grand scheme. Sometimes I think the
  authority stops sending /announce packets/ and it's difficult to know
  why because, it being a USB audio device, and not a USB serial
  device, it can't be serial monitored.
- A /Clients/ section which reports, as one might expect, on connected
  audio clients/clock subscribers. Clients send announce packets of
  their own, much like the time authority, and data from these packets
  populates the resulting table. The total number of clients is
  reported above the table, as is the /approximate group asymmetry/,
  which, given the above news regarding cascaded boundary clocks, may
  have to lean heavily on the word "approximate". Nonetheless, it's
  been useful for assessing presentation time offsets, packet buffer
  sizes, etc. as correspond to buffer fill levels; it's also shown me
  that the client has to work slightly harder for packets of 16 frames
  versus 32 (~0.7 % vs ~0.6 % CPU).

The monitoring side of things seems to be ticking along, clients
behave nicely, the plugin behaves nicely enough for now (it can
happily play-back/transmit pulses for hours without encountering a
catastrophic timestamp discrepancy). Time to write some audio code.

*** 2026-01-13 mardi
**** Meeting: Romain                                    :MEETING:SUPERVISION:
- Meet with Clemens at some point before I go on the 27th to bring him
  up to speed.
- Start manuscript while away --- which template?
  + Decide on this while away.
  + Ask Maxime what he used.
  + Romain to send his HDR template.
  + Basically, besides the front page, it's up to me...
- Journal article at some point; co-author with Clemens.
- Conference submissions.
- Fill in form for Marie Curie funding.
- Ask Cristina Rottondi about postdoc opportunity.
*** 2026-01-16 vendredi
**** WFS: two big problems            :SPATIALAUDIO:NETWORKEDAUDIO:TEENSY:
First up, introducing the WFS algorithm completely knackers the packet
buffer adjustment strategy. I repeatedly see serial reports like this:

#+begin_example
Presentation time diff 4005936 ns outside of range 0-666666 ns
Seeking closest packet... Found packet with presentation time diff 5936 ns (read index 22)
#+end_example

At the next 1588 interrupt something similar happens. Presentation
time is out by up to \pm10 \micro{}s.

Clients report a PTP lock and sensible sampling rates, there's no
significant additional weight of UDP traffic, and the entire module,
Ananas Client and WFS algo, is running at no more than 10% CPU (unless
~softwareISR~ and ~getOutputInterleaved~ in particular, are taking a very
long time). What's throwing off the presentation time delta so wildly?
Is there a delay reading the PTP timer?

Second problem: the WFS algorithm doesn't appear to run with more than
about 8 sources, and very modest soundfield dimensions, maximum
speaker spacing, etc. This suggests a memory problem. I can debug it
by sprinkling ~Serial.printf~ statements in the class derived from the
architecture file I created, but bloody hell this is concerning. It's
not a question of processing power, so I'm guessing it's delay
lines. Very disappointing. PSRAM required?
*** 2026-01-17 samedi
**** WFS: one problem fixed           :SPATIALAUDIO:NETWORKEDAUDIO:TEENSY:
Turns out the first problem was a fairly straightforward case of audio
processing causing a backlog of packets. I noticed that the average
packet reception interval was skewing slow, on the order of 700+ \micro{}s
instead of the expected 666 \micro{}s. Establishing a packet queue when
setting up a ~NetworkProcessor~ socket...

#+begin_src c++
qindesign::network::EthernetUDP socket{4};
#+end_src

...and switching from an if statement to a while loop in
~ananas::AudioClient::run~ did the job:
...
#+end_src
*** 2026-01-27 mardi
**** WFS: the other problem fixed; demo :FAUST:SPATIALAUDIO:NETWORKEDAUDIO:TEENSY:
By reducing the maximum speaker spacing to 30 cm, and adding =-dlt 1024=
to =faust2ananas= (thus reducing the size of the delay line buffers from
4096 to 2520), 16 sources works fine. Still, to handle focused sources
as well (via time reversal, see [cite:@ahrensAnalytic2012]), or more
than 16 secondary sources (i.e. more than eight modules), PSRAM will
almost certainly be required.

***** X-axis trouble
Plugged everything in the old salle commune and ran it and it wasn't
right. In switching to a unit square for the soundfield co-ordinate
space, and incorrectly deriving an equation for each module's delay
lines, I'd cocked things up. The solution:

#+begin_src faust :noeval
x(p) = hslider("%p/x", 0, -1, 1, 0.001) : +(1) : /(2) : *(maxX);
#+end_src

i.e. transposing \([-1, 1]\) to \([0, 1]\) means I could use the
existing code for the x-axis component of the right triangle between a
virtual source, \(y = 0\), and a given secondary source:
...
***** Parameter smoothing
Parameter smoothing outside of Faust is a bit of a mess, but that's
probably because I'm updating ~SmoothedParameter~ instances in ~loop()~
when in fact that should probably happen in
~AudioProcessor::processAudio()~.

***** Building and running the plugin on another machine
Next test was could I get things running on the behemoth Syfala
laptop, running Ubuntu (6.8.0-62-generic).
- Installing the VS Code platformIO extension required also installing
  =python3-venv= (see [[https://github.com/platformio/platformio-core-installer/issues/1774][this GitHub issue]]).
- That's fine and everything, but to use the helpful scripts in
  =teensy-audiosync= would require that the user installs [[https://koromix.dev/tytools][tytools]].
- Building the plugin entailed making some additions to the plugin's
  =CMakeLists.txt=; I had to add the following to ~target_link_libraries~
  #+begin_src cmake
target_link_libraries(ananasWFS
    ...
    ...
  #+end_src
  + This /might/ be because I had to change the CMake version to
    #+begin_src cmake
cmake_minimum_required(VERSION 3.28) # as opposed to 3.30
    #+end_src
    because that was the version installed on the Ubuntu
    machine. =cmake --version= on my Manjaro machine shows =cmake version
    4.2.1=. Could indeed be the problem there... aah, except CLion uses
    its own bundled CMake at version 3.30.5.
- Building the plugin also entailed installing =libcurl= (i.e. =sudo apt-get
  install libcurlpp-dev= IIRC)
- Running the plugin took some troubleshooting. Reaper was just
  hanging when I tried to add an instance of AnanasWFS; it turned out
  (confirmed by running the debugger in VS Code, and by using =strace=)
  that it was attempting to bind the ~TimestampListener~ socket to port
  320 that was the problem:
  #+begin_example
strace -e trace=socket,bind,setsockopt /home/syfala/opt/REAPER/reaper 2>&1
...
bind(36, {sa_family=AF_INET, sin_port=htons{320}, sin_addr=inet_addr("0.0.0.0")}, 16) = -1 EACCES (Permission denied)
  #+end_example
  Claude made some garbage suggestions about running =setcap= on the
  Reaper executable or the plugin, I had the thought that it might be
  a matter of the =syfala= user not being part of the =network= or =netdev=
  groups (the former not even existing by default on Ubuntu,
  apparently; Claude suggested I add it --- didn't help). Once I
  provided the above output to Claude it correctly suggested the
  following
  #+begin_src shell :noeval
# Check IP binding restrictions
sysctl net.ipv4.ip_unprivileged_port_start
  #+end_src
  Which returned 1024 on the Ubuntu machine, 0 on my Manjaro
  machine. I ran ~sudo sysctl net.ipv4.ip_unprivileged_port_start=0~ and
  added ~net.ipv4.ip_unprivileged_port_start=0~ to
  =/etc/system.d/10-network-security.conf= (I think that was the
  filename, in any case something in =/etc/system.d=) and that was that.

Entered on [2026-01-27 mar. 15:41]

** 2026-02 février
*** 2026-02-13 vendredi
**** Race conditions and stuff                 :NETWORKEDAUDIO:TEENSY:PTP:
Some progress made, with JUCE UI stuff, etc., but whenever I feel like
I'm getting somewhere with ironing out PTP sync bugs I find I haven't
cracked it.

The system seems to run fine as a plain server/client, but adding WFS
(and all the overhead that that brings to the clients) results in
sporadic loss of PTP lock that thus far I've been unable to resolve. I
"fixed" some potential race conditions in =t41-ptp= and =QNEthernet= (on
my own forks), but perhaps there are some left, because every so often
this sort of thing happens:

#+begin_example
==============================================================================
IP: 192.168.48.168 | MAC: 04:E9:E5:14:30:A8 | SN: 13231760
PTP t1: 9883201926468, t2: 9883201927757, t3 9883202490891, t4 9883202491726
  Mode: fine, Drift: 1127.454754, Offset: 54, Delay 1089, Adjust: 1017.454754
Clock Subscriber - Frames/Fs: 32/48000 (48000.0499418281906401)
PLL4 DIV: 29, NUM: 184030365, DENOM: 1000000000; SAI1 PRED: 3, PODF: 19
Audio-PTP offset: -22 ns
Ananas Client:     % CPU: 0.592000 (max 0.816000)
  Packets received: 249016 Average reception interval: 6.667828e+05 ns
  Read index: 45, Write index: 16, 42 % full.
  Packet offset: 216304 ns (10 frames), Times adjusted: 1
WFS:               % CPU: 23.087999 (max 23.808001)
  Module ID: 0, Speaker spacing: 0.228 m
Module:            % CPU: 23.952000 (max 29.679998)
==============================================================================


==============================================================================
IP: 192.168.48.168 | MAC: 04:E9:E5:14:30:A8 | SN: 13231760
PTP t1: 9885202617512, t2: 9885202257126, t3 9885202679007, t4 9885202679993
  Mode: coarse, Drift: 1127.454754, Offset: -180886, Delay -179700, Adjust: 0.000000
Clock Subscriber - Frames/Fs: 32/48000 (47999.9978400000036345)
PLL4 DIV: 29, NUM: 183998687, DENOM: 1000000000; SAI1 PRED: 3, PODF: 19
Audio-PTP offset: -45 ns
Ananas Client:     % CPU: 0.592000 (max 0.816000)
  Packets received: 250512 Average reception interval: 6.667805e+05 ns
  Read index: 42, Write index: 12, 40 % full.
  Packet offset: 216281 ns (10 frames), Times adjusted: 1
WFS:               % CPU: 23.120001 (max 23.808001)
  Module ID: 0, Speaker spacing: 0.228 m
Module:            % CPU: 23.983999 (max 29.679998)
==============================================================================


==============================================================================
IP: 192.168.48.168 | MAC: 04:E9:E5:14:30:A8 | SN: 13231760
PTP t1: 9886202620294, t2: 9886202621344, t3 9886203154265, t4 9886203155319
  Mode: fine, Drift: 1127.454754, Offset: -202, Delay 1052, Adjust: 0.000000
Clock Subscriber - Frames/Fs: 32/48000 (48000.0409440000003087)
PLL4 DIV: 29, NUM: 184024894, DENOM: 1000000000; SAI1 PRED: 3, PODF: 19
Audio-PTP offset: 853 ns
Ananas Client:     % CPU: 0.592000 (max 0.816000)
  Packets received: 252012 Average reception interval: 6.667797e+05 ns
  Read index: 39, Write index: 12, 46 % full.
  Packet offset: 216299 ns (10 frames), Times adjusted: 1
WFS:               % CPU: 23.280001 (max 23.808001)
  Module ID: 0, Speaker spacing: 0.228 m
Module:            % CPU: 24.240000 (max 29.679998)
==============================================================================


==============================================================================
IP: 192.168.48.168 | MAC: 04:E9:E5:14:30:A8 | SN: 13231760
PTP t1: 9887201857865, t2: 9887201859110, t3 9887202426241, t4 9887202427355
  Mode: fine, Drift: 1127.454754, Offset: -269, Delay 1045, Adjust: 1474.954754
Clock Subscriber - Frames/Fs: 32/48000 (48000.1259978281814256)
PLL4 DIV: 29, NUM: 184076607, DENOM: 1000000000; SAI1 PRED: 3, PODF: 19
Audio-PTP offset: 1150 ns
Ananas Client:     % CPU: 0.592000 (max 0.816000)
  Packets received: 253508 Average reception interval: 6.667817e+05 ns
  Read index: 36, Write index: 8, 44 % full.
  Packet offset: 216316 ns (10 frames), Times adjusted: 1
WFS:               % CPU: 23.280001 (max 23.808001)
  Module ID: 0, Speaker spacing: 0.228 m
Module:            % CPU: 24.224001 (max 29.679998)
==============================================================================
#+end_example

Note that Offset and Delay suddenly go high, and the subsequent
audio-PTP offset rises from a few nanoseconds to over a
microsecond. This is a relatively mild example. It happens for one
client at a time, and the switch doesn't appear to be affected, so
it's something to do with th

Depending on the severity of the loss of sync, a client may
resyncronise (to within a microsecond) in anything from a few seconds
to a few minutes... the trouble is, if PTP goes into coarse correction
mode, the audio-PTP offset enters into enormous oscillations that can take
ages to settle down.

***** PIO Elisp utilities 
With some thanks to Claude I have the following useful bits of Emacs
lisp for quickly monitoring 

***** Kill all PlatformIO monitor buffers

#+begin_src emacs-lisp :results none
(defun tar/kill-pio-monitors ()
  "Kill all PIO monitor buffers and their processes."
  (interactive)
  (dolist (buffer (buffer-list))
    (when (string-match "^\\*pio-monitor-ACM[0-9]+\\*" (buffer-name buffer))
...
***** Kill all PIO buffers and launch new ones

#+begin_src emacs-lisp :results none
(defun tar/setup-pio-monitors ()
  "Close all PIO monitor buffers and start 4 PlatformIO device monitors in a grid."
  (interactive)
  
  (tar/kill-pio-monitors)
  
  ;; Make the current window fill the frame
  (delete-other-windows)
  
  ;; Create the four monitors
  (let ((devices '(0 1 2 3))
        (buffers '())
        (display-buffer-alist '((".*" (display-buffer-no-window)))))
    
    ;; Start each async shell command
    (dolist (n devices)
      (let* ((cmd (format "pio device monitor -p /dev/ttyACM%d" n))
             (buffer-name (format "*pio-monitor-ACM%d*" n)))
    ...
    ;; Arrange in 2x2 grid
    ;; Top-left: first buffer (already displayed)
    (switch-to-buffer (nth 0 buffers))
    
    ;; Split right for top-right
    (split-window-right)
    ;; Split below for bottom left
    (split-window-below)
    ;; Switch to next window
    (other-window 1)
    ;; Open second shell command output
    (switch-to-buffer (nth 1 buffers))
    ;; Switch to next
    (other-window 1)
    ;; Open third buffer
    (switch-to-buffer (nth 2 buffers))
    ;; Split below
    (split-window-below)
    ;; Switch to next
    (other-window 1)
    ;; Open fourth
    (switch-to-buffer (nth 3 buffers))
    
    ;; Balance windows for equal sizes
    (balance-windows)))
#+end_src

Entered on [2026-02-13 ven. 13:59]

*** 2026-02-22 dimanche
**** QoS, IPs/ports, QNEthernet, focused sources, etc. :NETWORKEDAUDIO:TEENSY:PTP:
Latest developments (I've been working quite obsessively and not
taking notes):
- It turns out Quality of Service is quite important, and there's a
  [[https://help.mikrotik.com/docs/spaces/ROS/pages/189497483/Quality+of+Service#QualityofService-Dante][guide]] in the MikroTix Router OS docs about configuring Dante; I
  adapted this to my setup, but I think I need to refine my approach.
  + This is all with the aim of prioritising PTP traffic, then audio
    traffic, and leaving announcemeguident traffic and control data as
    low-priority.
  + I realised that I was getting a pretty stable PTP lock for
    AnanasServer (versus AnanasWFS) because although the packet rate
    was a comparable ~1500/sec, bandwidth was 2 Mbps vs 12 Mbps;
    follow-up messages weren't reaching the clients from the switch
    boundary clock, presumably because they were being overwhelmed by
    audio traffic.
- Switching on QoS stuff on the switch broke everything --- this was
  because clients weren't sending IGMP packets and were thus getting
  kicked out of their multicast groups. I spent a couple of hours
  updating my fork-of-a-fork of QNEthernet on the hunch that the
  canonical upstream had been fixed in the intervening five years, and
  I was right.
- I also rewrote the PTP implementation from scratch. It only supports
  IPv4 E2E, but that's all we need. It's much easier to maintain, it's
  more efficient, I think, and I understand PTP a lot better now than
  I did before.
- I had a go at simplifying the protocol part of my implementation; I
  now have the following IP:port combinations
  + Audio data on =224.4.224.4:49152=
  + WFS control data on =224.4.224.5:49162=
  + Reboot messages on =224.4.224.5:49163=
  + Authority accouncements on =224.4.224.6:49172=
  + Client accouncements on =224.4.224.6:49173=

Entered on [2026-02-22 dim. 19:01]
*** 2026-02-23 lundi
**** IGMP addendum                                 :NETWORKEDAUDIO:TEENSY:
The MikroTik Dante QoS guide says
#+begin_quote
When using Dante in multicast mode, it is beneficial to enable IGMP snooping on the switch. This feature directs traffic only to ports with subscribed devices, preventing unnecessary flooding.
#+end_quote
I narrowed down some problems I was having with clients dropping off
the network to IGMP snooping being enabled. It's kind of useful for
verifying which devices are subscribed to which multicast groups, but
appears to be more trouble than it's worth.

Entered on [2026-02-23 lun. 21:01]
** 2026-03 mars
*** 2026-03-03 mardi
**** PTP fix, PI controller, ongoing issues    :NETWORKEDAUDIO:TEENSY:PTP:
Trynig to take sync measurements with =ananasServer=, I found that,
after about forty-five minutes, clients started to drift. My idea was
that perhaps I had a (16-bit?) counter that was wrapping, but it
turned out that the issue was the delay request sequence ID,
i.e. without a propertly incrementing senquence ID, eventually the
switch was just not responding to delay request messages, leaving time
exchanges incomplete, and causing ~updateController~ not to be called,
and, consequently, no update to the ~audioPTPOffset~ PI controller that
I recently added

***** PI controller
I switched to a PI controller for handling the ~audioPTPOffset~
...
where ~KP~ and ~KI~ are ~.1~ and ~.01~ respectively. This replaced a previous
attempt using a mean sampling rate as a fallback when sync was lost,
that obviously wasn't working. My thinking with the previous iteration
was that, after a little while at least, the average sampling rate
should be analogous to PTP drift and remain reasonably constant, thus
getting around situations whereby large PTP adjusments are made, and
thus large ~audioPTPOffset~ values are calculated. The averaging
approach wasn't helping matters, however, whereas the PI controller
does. PTP trouble, particularly when running =ananasWFS= (and
=wfs-module=) still cause async, but the PI controller does appear to be
effective in bringing things back into line. Possible further
improvements available by tweaking ~KP~ and ~KI~.

***** Ongoing issues
The switch is the weakpoint as far as I can tell. When running
=ananasServer= and =clock-subscriber=, follow-up message loss --- i.e. the
server sending a sync message, but not the corresponding follow-up to
a given clock subscriber --- is on the order of < 1 %, which isn't
enough to cause much of a problem. When running =ananasWFS= and
=wfs-module=, however, loss is over 10 %, which is occasionally rather
catastrophic for presentation-time synchronicity. The aforementioned
PI controller does its job, but it can take a minute or two for clock
subscribers/modules to resynchronise if they're knocked out of line by
a particularly egregious loss of PTP exchange data.

The plugin UI is coming together, but for =ananasWFS= in particular
there remains the issue whereby if a module disconnects all other
modules' iDs are reset to -1, and, upon rejoining, they don't receive
their former module IDs. Modules also don't appear to be correctly
parsing (or receiving?) initial source position OSC data when they
connect, i.e. subsequent source position changes are required to
/remind/ modules of source positions.

Entered on [2026-03-03 mar. 07:29]

*** 2026-03-10 mardi
**** Meeting: Romain                                 :MEETING:SUPERVISION:
- Start the thesis from the middle... something very technical. Build
  outwards.
- Sketch an outline... what's going to be the table of contents?
  Romain & Tanguy need to sign this off.
- Reasonable length ~100-130 pages. Can be more (ideally not more
  than 200) shouldn't be less.
- For ~130 pages, technical background should be around 15 pages.
- State of the art should be really exhaustive, not just on
  distributed audio, but in /all directions/. Networked audio, PTP, etc.
- Be as broad as I like, but be concise.
- Keep the style simple.
- Really important: one thing many theses lack is a very clear
  exhibition of the scientific challenges of the work.
  + /Research question/ approach maybe.
- Highlight the contributions that I've made; very explicitly.
  + Outline them in the intro or pre-intro as /Contributions/ or
    /Foreword/.
- Romain to send me a good thesis he evaluated recently.

Entered on [2026-03-10 mar. 16:08]
*** 2026-03-13 vendredi
**** Ambisonics paper                                            :WRITING:
- Spatial/immersive audio desirable but out of reach, costly, etc.
- HOA requires high channel-count transmission and many loudspeakers.
- IEM plugins exist, which is good.
- Combined with our networked audio plugin and a distributed decoder,
  it's possible to produce 5th order ambisonics \((5+1)^{2}\) channels of
  b-format need to be transmitted... to something like 64
  loudspeakers, so 32 modules
  + So switch daisy-chaining required, and commensurate temporal
    offset (~100 ns).
  + 4th order entails 25 channels and, ideally 48 speakers... could
    probably bring that down to 44, which would be the limit with one
    switch, accounting for the fact that two ports are taken up by the
    time authority and the DAW host respectively (\(24 - 2\) ports
    gives \(22 \times 2 = 44\) loudspeakers).
- Anyway, we sent our 25 channels of b-format encoded audio over the
  network to each of our 22 Teensy modules.
  + Each teensy was informed of the two rows of decoding coefficients
    needed in order to produce its parts of the soundfield.
Entered on [2026-03-13 ven. 20:09]
*** 2026-03-28 samedi
**** Thesis thoughts                                   :WRITING:BRAINDUMP:
- I haven't been taking enough notes these past few months.
  + I can probably cut myself some slack for that.
  + I wrote a paper for SMC, so that's something.
- I think three of my chapters should be /Networked Audio/, /Time
  Exchange/, and /Sound Field Synthesis/.
- I think /Networked Audio/ should start with networking in general;
  definition/origins of packet-switched (as opposed to
  circuit-switched) networks; the fact that Ethernet is fifty years
  old and still bears the same name and basic packet structure; 90's
  VOIP, SoundWire, JackTrip, etc.
- /Time Exchange/ ought to start with something about ppm and the fact
  that drift is inevitable; no two reproductions of the same audio
  information (whether stored/transferred digitally or anologue) will
  ever be the same, sort of as a justfication for using any old Teensy
  as the time authority.
- /Sound Field Synthesis/ should obviously describe WFS and HOA, but
  also how we got there I suppose. Ambisonics is surprisingly old
  (30's?). Driving functions? I feel like taking Jens's approach,
  which does a lot of assuming continuous secondary arrays, is too
  theoretical for my work, but I'll have to look into it further.

Regarding drift, I tried to find out what XO is driving the Teensy
4.1. The best I could discern is that the IMXRT1060 evaluation board
uses a [[https://www.mouser.fr/datasheet/3/827/1/7v.pdf][TXC 7V24000002]] XO (alluded to [[https://forum.pjrc.com/index.php?threads/teensy-4-testing-mbed-nxp-mxrt1050-evkb-600-mhz-m7.54265/page-7#post-193228][here]], in the PJRC forum). There's
some more info in this [[https://claude.ai/chat/eb7b4563-5115-4eb0-8f1f-8e1406032352][chat]] with Claude. In any case, all I have a
basic idea of for the time being is that the XO is rated to [[https://forum.pjrc.com/index.php?threads/teensy-4-1-as-stable-clock-source.71539/#post-316465][30 ppm]],
but I don't have a /proper/ source beyond what Paul Stoffregen says on
the forum.

Purely anecdotally, based on what I can recall seeing the switch
reporting re. PTP drift, 17 ppm (or rather 17,000 ppb) is a number
that jumps to mind, but I guess I should try syncing to a bunch of
different T41s programmed as the time authority and see what sort of
drift figures emerge.

Entered on [2026-03-28 sam. 20:53]
*** 2026-03-31 mardi
**** Things to talk about in the manuscript            :WRITING:BRAINDUMP:
In no particular order, just to start getting this stuff out of my
head and into the bloody Org file.

- The Open Systems Interconnection (OSI) model and/or the Internet
  Protocol Suite
- User Datagram Protocol (UDP, transport layer)
- Multicast group
- Internet Group Membership Protocol (IGMP, internet layer)
...
- Teensy 4.1 (and other versions)
- Raspberry Pi/Bela/Elk/ELS
- Daisy/STM32/ESP32
- Precision Time Protocol (PTP, application layer)
- Network Time Protocol (NTP, application layer)
- The importance of synchronisation in audio systems
- Ananas, of course
- The Ananas Audio Protocol (AAP, application layer)
- The Ananas Client Description Protocol (ACDP, application layer)
- The Ananas Authority Description Protocol (AADP, application layer)
- Atomic clocks/GPS
- Clock Drift
- Jitter?
- Faust
...
- Encoding audio (and arbitrary) data to network packets
...
- The NXP IMXRT1062 MCU
- Algorithm for finding ideal register values for achieving target
  sampling rate on the IMXRT1062
...
- IP addresses
- IPv4
- Medium/media access control (MAC, link layer)
...
- Arduino?
- USB Audio & feedback accumulation
- PlatformIO
- Psychoacoustics/localisation
- Spatial Aliasing
- Secondary Point Sources

Entered on [2026-03-31 mar. 21:37]
** 2026-04 avril
*** 2026-04-01 mercredi
**** What's a Phase Locked Loop?                        :WRITING:HARDWARE:
#+begin_quote
A PLL is a device which causes one signal to track another one.
#+end_quote

Continuing\nbsp[cite:@hsiehPhaselocked1996]:

#+begin_quote
It keeps an output signal synchronizing with a reference input signal
...
#+begin_quote
More precisely, the PLL is simply a servo system, which controls the
phase of its output signal in such a way that the phase error between
output phase and reference phase reduces to a minimum.
#+end_quote

Or, as Claude put it:

#+begin_quote
A Phase-Locked Loop (PLL) is an electronic feedback circuit that locks
the phase of an output oscillator to match that of a reference input
signal, allowing it to generate a stable output frequency that is a
precise multiple of the reference.
#+end_quote

It is interesting that there's a parallel here between a PLL and
PTP. I'm sure I've seen the word "servo" thrown around where PTP is
concerned. PI controllers too. All to do with time and/or frequency
and synchronisation.

Entered on [2026-04-01 mer. 20:10]
*** 2026-04-02 jeudi
**** MCSA Postdoctoral Fellowship Training                      :TRAINING:
- Quick info-dump on the program...
- Checking eligibility is a goal for the day
- Session 1: Intro to MCSA PF, 21/04 10h -- 12h30, online
- Session 2: How to write a proposal for PF, 19/05 9h -- 16h30, online
- Session 3: How to fine tune your proposal, 22-26/06 Paris, in-person
- *Register for the above sessions* https://sondages.inria.fr/index.php/745553?lang=en
- June-Aug, CPPI Europe will support in preparing proposal
- Deadline September 9th
- MSCA is an EC program for research and innovation
  + Focus on mobility --- international, intersectoral
    (academic/industrial), interdisciplinary
  + Only program that is for individuals (as opposed to institutions)
- For PhD degree holders.
  + Hm, I'm probably not eligible this year because I won't defend until
    October...
- Funding is incredibly generous. (Hence highly competitive.)
  + Possible to /hire/ e.g. a masters student for an internship with
    part of the funding.
- Possible start date April 2027 (deferrable up to August 2028)
- Mobility rule: not resident in country of destination more than 12
  months in the 36 months prior to deadline.
- Postdoctoral Fellowship (PF), Global Fellowship (GF)
- European vs /Global/ fellowships. The latter more complex to write;
  three years in total (instead of two), two outside of Europe (the
  list of countries that qualify as Europe is... weird, e.g. Egypt).
- Anyway, bottom-up project, training-through-research; individual,
  personalised; transferable skills, teaching, dissemination,
  communication/outreach, "horizontal skills", training in gender and
  ethics issues.

- Maybe attend first session this year, but not the second
- Ask Romain about writing a "paper-thesis"...
- Generally mention that I came here and I'm interested in the
  process.
- Maybe if there's the possibility of staying with the team for a
  postdoc...
- Even if I go somewhere else for a postdoc, I can apply with Inria
  for PF/GF next year.
- Stanford is an option... choose a supervisor with different
  expertise cf. Romain/Emeraude.
- Grame could serve as 6-month placement

Entered on [2026-04-02 jeu. 09:54]
*** 2026-04-06 lundi
**** I've been reading about networks                            :WRITING:
What have I actually learned as a result of reading (most
of)\nbsp[cite:@bonaventureComputer2011]? Well, that the Ethernet MTU is
1500 bytes, which I already knew; I still don't really know why, other
than that it's an arbitrary limit and was arrived at by
experimentation I suppose. That switches are data link layer
relays. That the OSI model and TCP/IP suite are actually mortal
enemies (see also\nbsp[cite:@russellRough2006]). Perhaps it wasn't the
most fruitful way to spend several days, i.e. fretting over forcing
myself to read something dull, with ugly figures and occasional
typos. Not the strongest of bibliographical entries for the thesis.

Entered on [2026-04-06 lun. 22:00]
*** 2026-04-16 jeudi
**** Reliable PTP achieved?                           :NETWORKEDAUDIO:PTP:
Let's not get carried away, but it looks like the development version
of Router OS that MikroTik support provided has done the
trick... mostly. Anecdotally, we've gone from > 30 % follow-up packet
loss at 44 channels (~ 35 Mbps Rx) to ~1 %. The remaining follow-up
losses don't appear to be catastrophic enough to cause
desynchronisation, even for that high number of channels and
corresponding high(ish) Rx bandwidth.

So, with that out of the way (for now... I must test it again
tomorrow), it's back to Faust. Latest headache is the WFS algorithm,
or something in the Faust-generated C++ at any rate, is causing
intermittent clicks. Another one to re-test tomorrow. Things I've
tried:
- Running only 8 virtual sources instead of 16 (still clicks)
- Running only /2/ virtual sources (clicks)
- Commenting out ~wfs.processAudio~ (clean output results)
- Commenting out ~compute~ in =wfs.cpp= and just copying the input to the
  output (clean)

I should try a /real/ Faust passthrough , ~_~, rather than a hack, and see
what happens. 

Entered on [2026-04-16 jeu. 16:17]
*** 2026-04-17 vendredi
**** Still clicking... or not                      :NETWORKEDAUDIO:TEENSY:
So it doesn't appear to be:
- Where Faust's float buffers are located in memory, nor whether
  they're dynamically or statically allocated.
- The hierarchy of interrupt priorities on the Teensy.
- Clipping.
- Whether focused source computation is happening as well as virtual
  source computation.
- The =-dlt= flag.
- ~static_cast~ usage in ~compute~.

I tried setting up a /passthrough/ environment, and that works just fine
--- no clicking. As low as two input channels (WFS sources) and
there's intermittent clicking audible when reproducing a 240 Hz sine
wave.

I tried removing the questionable ~distanceSim~ I implemented, and... no
clicking. What the bloody hell?

Entered on [2026-04-17 ven. 15:37]
*** 2026-04-21 mardi
*** 2026-04-22 mercredi
**** Reading, WFS second version                            :SPATIALAUDIO:
Save the end of the final appendix, I finished Jens's
book\nbsp[cite:@ahrensAnalytic2012]. Did I learn much of practical
pertinence to my work? Maybe not. Perhaps that I should be windowing
the edges of my WFS array to mitigate for truncation artifacts.

I decided that the module ID approach, and each WFS module being aware
of the number of modules in the array, isn't the right way to
go. Rather, modules should just be aware of the co-ordinates of the
secondary sources for which they're responsible and the distributed
algorithm should take care of the rest. I'm confident that this is a
solid rationale, but it calls for a fairly significant rewrite of the
control system to account for the fact that modules expect to receive
secondary source co-ordinates instead of a module ID. Makes a stronger
case for /unicasting/ module-specific parameters, i.e. secondary source
co-ordinates, e.g. OSC over UDP addressed to a given IP address and
...
***** Clicking hell
The clicking is back, whether or not the WFS prefilter is active. I
really don't know what's causing the problem. I tried writing a WFS
algorithm from scratch in C++, doing all the computation on ~int16_t~
samples, and the clicking remained. Also CPU time was hovering around
30% (without the prefilter) versus 26-27% for the Faust implementation
(with the prefilter). So I guess sticking with Faust is the way to
go. My thinking was it couldn't be good to be converting ~int16_t~ to
~float~, doing DSP, then converting back (i.e. perhaps it was the FPU
creaking under the weight of all the computation), but that turns out,
as far as I can tell, not to be the case.

Entered on [2026-04-22 mer. 18:18]

*** 2026-04-24 vendredi
**** Ethernet interface setup                 :PROGRAMMING:NETWORKEDAUDIO:
Agh, so I spent half a day trying to unicast secondary source
co-ordinates to a module, each time finding that my OSC message was
being sent over wifi instead of ethernet. It turns out the problem was
the netmask/address range available to ethernet.

If I want to sent a UDP unicast packet on interface ~192.168.10.10~ to,
e.g. ~192.168.48.203~, the netmask should be ~255.255.0.0~, not
~255.255.255.0~, i.e. ~192.168.10.10/16~.

Anyway, now it works; I just need to set up the module to receive the data.

Entered on [2026-04-24 ven. 10:56]
** 2026-05 mai
*** 2026-05-05 mardi
**** Clicking resolved                             :NETWORKEDAUDIO:TEENSY:
Cause: out-of-sequence packets:

[[./images/20260505/WhatsApp Image 2026-05-05 at 10.52.51.jpeg]]

So, thats 16 frames being reproduced 32 frames late.

Solution: buffer write-index shenanigans:
#+begin_src c++
...
The two main scenarios I'm seeing are

#+begin_example
Seq. num.: 1  2  3  5  4  6  7  8  9
    diff.:   +1 +1 +2 -1 +2 +1 +1 +1
#+end_example

and (more rarely)

#+begin_example
Seq. num.: 1  2  3  6  5  4  7  8  9
    diff.:   +1 +1 +3 -1 -1 +3 +1 +1
#+end_example

My algorithm appears to handle both without any trouble.

Entered on [2026-05-05 mar. 14:14]
*** 2026-05-10 dimanche
**** Abstract and bio for Turin presentation                :PRESENTATION:
***** Accessible Spatial Audio Installations on a Network of Distributed Signal Processors
Access to state-of-the-art spatial audio systems is limited by
techical and economic factors, with highly multichannel audio
interfaces and related equipment typically being very costly and
inflexible. The rise of networked audio, embedded computing, and
embedded audio in particular, plus the /write once, run anywhere/ model
of the Faust programming language, mean that the technical ingredients
exist for democratising spatial and immersive audio applications.

This talk will present the technical challenges faced by the idea of
running spatial audio algorithms in a distributed setting. The search
for a suitable candidate hardware platform is principal amongst these,
and is closely tied to the challenge of synchronising signal
processors in a networked audio context. The parallelisation of wave
field synthesis and higher order ambisonics in Faust are the main
algorithmic challenges to be addressed. The development of a new
networked audio protocol for time-sensitive applications is also
discussed.
***** Bio
Thomas Rushton received his MSc in Sound and Music Computing at
Aalborg University, Copenhagen. As part of this program, he worked as
an intern with Inria's Emeraude (Embedded Programmable Audio Systems)
research team in Villeurbanne, France, developing a distributed
spatial audio system based on the Faust programming language, the
Teensy 4.1 microcontroller, and the JackTrip networked audio
protocol. He rejoined Emeraude in 2023 as began working toward a PhD
at INSA Lyon with the tentative title /Enabling Distributed Spatial
Audio/. His principal areas of research are accessible, distributed
spatial audio systems and differentiable DSP, topics upon which he has
presented at a number of conferences across Europe over the past few
years.

Entered on [2026-05-10 dim. 11:46]
*** 2026-05-18 lundi
**** PTP symbols                                             :WRITING:PTP:
I never particularly enjoyed using, e.g. \(\text{Offset}\) as the name
of a variable. Particularly when bringing the audio-PTP offset into
the picture things are going to get messy. Consulting with Claude, it
turns out that Mills, in his paper on NTP v3\nbsp[cite:@millsImproved1994]
uses \(\theta\) for offset, \(\delta\) for delay, and Claude suggested \(\alpha\) for
drift. I intend to use \(\Delta\) for the audio-PTP offset (it's a /delta/ of
some kind).

Entered on [2026-05-18 lun. 13:30]
** 2026-06 juin
*** 2026-06-25 jeudi
**** Meeting: Tanguy & Romain                         :MEETING:SOUTENANCE:
- State of the art should include sound field synthesis.
- Developing digital audio systems should be chapter 2. Talk about
  Faust.
- Could have a chapter on autodiff in Faust as chapter 3.
- Spatial audio sync and time exchange.
- Ananas

Entered on [2026-06-25 jeu. 11:01]
